...
import { WarningCircle, Warning, CheckCircle, DownloadSimple, Link, Spinner, Table } from '@phosphor-icons/react'
            ...
            footer={
                <div className="flex items-center w-full gap-2">
                    <button
                        ...
                        title="DownloadSimple Format Excel/CSV"
                    ...
                    >
                        <DownloadSimple className="opacity-80 group-hover:animate-bounce w-3 h-3" />
                        <span>Template</span>
                    </button>
                    ...
                    >
                        {fetchingGSheets ? (
                            <><Spinner className="fa-spin w-3 h-3" /> <span>Sinkron...</span></>
                        ) : (
                            <><Link className="opacity-80 w-3 h-3" /> <span>Hubungkan</span></>
                        )}
                    </button>
                </div>
            }
            ...
            <div className="space-y-6">
                ...
                <div className="relative overflow-hidden p-4 rounded-2xl bg-emerald-500/[0.04] border border-emerald-500/10 flex gap-4 items-start shadow-sm group">
                    <div className="absolute top-0 right-0 p-2 opacity-10 group-hover:scale-110 transition-transform">
                        <CheckCircle size="3x" className="text-emerald-500" />
                    </div>
                    <div className="w-10 h-10 rounded-xl bg-white dark:bg-gray-800 shadow-sm flex items-center justify-center shrink-0 border border-emerald-500/20">
                        <WarningCircle className="text-emerald-500 w-4 h-4" />
                    </div>
                    <div className="space-y-1">
                        ...
                        <p className="text-[11px] font-bold text-emerald-800/70 dark:text-emerald-300/70 leading-relaxed">
                            Buka menu <b className="text-emerald-900 dark:text-emerald-200">ShareFat</b> di GSheets, lalu ubah General Access menjadi <span className="bg-emerald-500/20 px-1.5 py-0.5 rounded-md text-emerald-700 dark:text-emerald-300">Anyone with the link</span> agar sistem dapat membaca data.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between px-1">
                        ...
                        {urlStatus === 'valid' && <span className="text-[9px] font-black text-emerald-500 uppercase flex items-center gap-1 animate-in fade-in slide-in-from-right-2"><CheckCircle /> Link Terdeteksi</span>}
                        {urlStatus === 'invalid' && gSheetsUrl && <span className="text-[9px] font-black text-red-500 uppercase flex items-center gap-1 animate-in fade-in slide-in-from-right-2"><Warning /> Bukan Link GSheets</span>}
                        {urlStatus === 'incomplete' && <span className="text-[9px] font-black text-amber-500 uppercase flex items-center gap-1 animate-in fade-in slide-in-from-right-2"><Spinner className="fa-spin" /> Menunggu Link Lengkap</span>}
                    </div>
