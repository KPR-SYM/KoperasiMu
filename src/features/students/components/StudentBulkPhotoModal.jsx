...
export default function StudentBulkPhotoModal({
    ...
    const buildStudentOptions = (fileName) => {
        ...
        const scored = allStudentsForBulk.map(s => {
            ...
            return { id: s.id, name: s.name, icon: User, score }
        })
                ...
                <div className="flex items-center w-full gap-3">
                    ...
                    {bulkPhotoMatches.length > 0 && (
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {uploadingBulkPhotos ? <><Spinner className="fa-spin" /> Mengupload...</> : <><Check /> Simpan Foto</>}
                            </button>
                        </div>
                    )}
                </div>
        ...
        >
            <div className="space-y-5">
                ...
                <label className="flex items-center gap-3 p-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)]/30 cursor-pointer hover:bg-[var(--color-surface-alt)]/60 transition-colors select-none">
                    <div className={`w-9 h-5 rounded-full relative transition-colors ${autoShrink ? 'bg-[var(--color-primary)]' : 'bg-[var(--color-border)]'}`}>
                        <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${autoShrink ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                        <div className="text-[10px] font-black uppercase tracking-wider text-[var(--color-text)]">
                            <ArrowsInSimple className="mr-1.5 text-[var(--color-primary)]" />
                            Auto Kompres Gambar
                        </div>
                        <div className="text-[9px] text-[var(--color-text-muted)] mt-0.5">Kompres ke max 800Ã—800px, kualitas 70% (hemat storage)</div>
                    </div>
                </label>
                ...
                <div className={`border-2 border-dashed border-[var(--color-primary)]/30 bg-[var(--color-primary)]/5 flex flex-col items-center text-center group hover:border-[var(--color-primary)] hover:bg-[var(--color-primary)]/10 transition-all cursor-pointer relative rounded-2xl overflow-hidden ${bulkPhotoMatches.length > 0 ? 'p-4' : 'p-8'}`}>
                    <input
                        ...
                        onChange={(e) => handleBulkPhotoMatch(e.target.files, matchMethod, autoShrink)}
                        disabled={uploadingBulkPhotos || matchingPhotos}
                    />
                    {matchingPhotos ? (
                        <div className="flex items-center gap-3 py-1">
                            <Spinner className="fa-spin w-5 h-5 text-[var(--color-primary)]" />
                            <span className="w-3 h-3 font-bold text-[var(--color-text-muted)]">Mencocokkan foto...</span>
                        </div>
                    ) : bulkPhotoMatches.length > 0 ? (
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                                <div className="w-3 h-3 font-black text-[var(--color-text)]">Tambah File Foto Lain</div>
                        ...
                        <>
                            ...
                            <h4 className="w-4 h-4 font-black text-[var(--color-text)] mb-1">
                                        ...
                                        {bulkPhotoMatches.map((item, i) => (
                                            <tr key={i} className="hover:bg-[var(--color-surface-alt)]/50 transition-colors group" style={{ contentVisibility: 'auto', containIntrinsicSize: '0 56px' }}>
                                                ...
                                                <td className="p-3">
                                                    ...
                                                    {autoShrink && item.originalSize !== item.compressedSize && (
                                                        <div className="text-[8px] text-emerald-600 font-bold mt-0.5">
                                                            {formatSize(item.originalSize)} â†’ {formatSize(item.compressedSize)}
                                                        </div>
                                                    )}
                                                </td>
                                                <td className="p-3">
