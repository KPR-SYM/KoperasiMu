...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const { error } = await signIn(username, password, false);

      if (!error) {
        toast.success('Login Berhasil!');
        navigate('/dashboard');
      } else {
        toast.error(error.message || 'Login Gagal! Periksa username atau password.');
      }
    } catch (err) {
      ...
      <div className="flex-1 flex items-center justify-center relative overflow-hidden p-4">
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          >
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl border border-transparent shadow-lg shadow-blue-500/25 dark:shadow-blue-500/10 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-70 disabled:cursor-not-allowed disabled:hover:translate-y-0 transition-all duration-200 transform hover:-translate-y-0.5"
              >
                {loading ? (
          ...
          <div className="flex items-center justify-center gap-4 mt-6 px-2">
            <Link to="/check" className="text-xs font-bold text-gray-400 dark:text-white/30 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
              Cek Data Santri
            </Link>
            <span className="w-1 h-1 rounded-full bg-gray-300 dark:bg-white/20" />
            <Link to="/" className="text-xs font-bold text-gray-400 dark:text-white/30 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
              Beranda
            </Link>
          </div>
        </div>
