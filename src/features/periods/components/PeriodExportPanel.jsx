...
const COLUMN_DEFS = [
    ...
    { key: 'is_active', label: 'Status Aktif', icon: CheckCircle, exportKey: 'Status Aktif' },
    { key: 'is_locked', label: 'Status Kunci', icon: Archive, exportKey: 'Status Kunci' },
]
...
const EXPORT_FORMAT_CONFIG = [
    ...
    { label: 'iCal', icon: Calendar, desc: '.ics', color: 'hover:border-blue-400 hover:bg-blue-50', iconColor: 'text-blue-500', format: 'ics' },
]

const FORMAT_ICONS = { csv: FileXls, excel: FileXls, pdf: FileText, ics: Calendar }

export default function PeriodExportPanel(props) {
    const {
        isOpen,
        onClose,
        years = [],
        ...
        if (exportPhase === 'loading' && !exporting) {
            if (exportError) {
                ...
                const timer = setTimeout(() => {
                    ...
                    addToast('File berhasil diunduh', 'success')
                }, remaining)
                return () => clearTimeout(timer)
            }
        }
    }, [exporting, exportPhase, exportError, addToast])
    ...
    const handleExport = useCallback((format) => {
        const handlers = { csv: handleExportCSV, excel: handleExportExcel, pdf: handleExportPDF, ics: handleExportICS }
        const handler = handlers[format]
        if (!handler) return
        setExportFormat(format)
        setExportPhase('loading')
        exportStartRef.current = Date.now()
        const options = { includeHeader, orientation: pdfOrientation, template: exportTemplate }
        handler(fileName, options)
    }, [handleExportCSV, handleExportExcel, handleExportPDF, handleExportICS, includeHeader, pdfOrientation, exportTemplate, fileName])
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {COLUMN_DEFS.map(({ key, label, icon: Icon }, idx) => {
                        ...
                        return (
                            <button
                                ...
                                onDrop={(e) => { e.preventDefault(); handleColumnDrop(idx) }}
                                ...
                                className={`group relative flex items-center gap-2 px-2.5 py-2 rounded-xl border text-left transition-all
                                    ...
                                    ${dragIdx === idx ? 'opacity-40 ring-2 ring-[var(--color-primary)] scale-95' : ''}
                                `}
                            >
        ...
        <div className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-[var(--color-surface-alt)]/50 border border-[var(--color-border)]">
                <div className="text-[10px] font-bold text-[var(--color-text-muted)]">
                    Ringkasan: <span className="font-black text-[var(--color-text)]">{exportPreviewData.total}</span> periode akan diekspor sebagai <span className="font-black text-[var(--color-text)]">{exportColumns.length}</span> kolom
                </div>
            </div>

            {hasColumns && exportPreviewData.rows.length > 0 && (
                <div className="space-y-3">
                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-muted)]">Preview Format</p>

                    <div className="rounded-2xl border border-[var(--color-border)] overflow-hidden bg-[var(--color-surface)]">
                        <div className="flex gap-1 p-1 bg-[var(--color-surface-alt)] border-b border-[var(--color-border)]">
                            {['csv', 'excel', 'pdf', 'ics'].map(f => (
                                <button
                                    key={f}
                                    onClick={() => setPreviewFormat(f)}
                                    className={`flex-1 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${previewFormat === f ? 'bg-[var(--color-surface)] text-[var(--color-text)] shadow-sm' : 'text-[var(--color-text-muted)] hover:bg-[var(--color-surface)]/50'}`}
                                >
                                    {f === 'ics' ? 'iCal' : f}
                                </button>
                            ))}
                            ...
                            {previewFormat === 'csv' && (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 mb-2">
                                        <FileXls className="w-3 h-3 text-slate-500" />
                                        <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">CSV Preview</span>
                                        <span className="text-[8px] font-bold text-[var(--color-text-muted)] opacity-50">— comma-separated values</span>
                                    </div>
                                    <pre className="bg-slate-50 rounded-xl p-3 text-[10px] font-mono text-slate-700 overflow-x-auto border border-slate-200">
{includeHeader && <span className="text-slate-400">{exportColumns.map(k => COLUMN_DEFS.find(c => c.key === k)?.exportKey || k).join(',')}{'\n'}</span>}
{exportPreviewData.rows.map(row => exportColumns.map(k => {
    const val = row[COLUMN_DEFS.find(c => c.key === k)?.exportKey || k] ?? ''
    return `"${String(val)}"`
}).join(',')).join('\n')}
{exportPreviewData.total > 5 && <span className="text-slate-400">... +{exportPreviewData.total - 5} baris lagi</span>}</pre>
                                </div>
                            )}
                            ...
                            {previewFormat === 'excel' && (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 mb-2">
                                        <FileXls className="w-3 h-3 text-emerald-500" />
                                        <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">Excel Preview</span>
                                        <span className="text-[8px] font-bold text-[var(--color-text-muted)] opacity-50">— .xlsx spreadsheet</span>
                                    </div>
                                    <div className="overflow-x-auto">
                                        <table className="w-full border-collapse text-[10px]">
                                            {includeHeader && (
                                                <thead>
                                                    <tr className="bg-emerald-50">
                                                        {exportColumns.map(k => (
                                                            <th key={k} className="px-3 py-2 border border-emerald-200 text-left font-black text-emerald-700 uppercase tracking-wider">{COLUMN_DEFS.find(c => c.key === k)?.exportKey || k}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                            )}
                                            <tbody>
                                                {exportPreviewData.rows.map((row, ri) => (
                                                    <tr key={ri} className="hover:bg-emerald-50/30">
                                                        {exportColumns.map(k => (
                                                            <td key={k} className="px-3 py-2 border border-emerald-200 font-medium">{row[COLUMN_DEFS.find(c => c.key === k)?.exportKey || k] ?? ''}</td>
                                                        ))}
                                                    </tr>
                                                ))}
                                            </tbody>
                                    ...
                                    {exportPreviewData.total > 5 && <div className="text-[9px] font-bold text-[var(--color-text-muted)] opacity-50 text-center py-1">... +{exportPreviewData.total - 5} baris lagi</div>}
                                </div>
                            )}
                            ...
                            {previewFormat === 'pdf' && (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">PDF Preview</span>
                                        <span className="text-[8px] font-bold text-[var(--color-text-muted)] opacity-50">— {pdfOrientation} • template {exportTemplate}</span>
                                    </div>
                                            ...
                                            <div className="space-y-1.5">
                                                {exportPreviewData.rows.slice(0, 3).map((row, ri) => (
                                                    <div key={ri} className="flex items-center gap-2 text-[8px]">
                                                        ...
                                                        <span className="text-rose-400">•</span>
                                                        <span className="text-[7px] text-rose-400">{row['Mulai']} s/d {row['Selesai']}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        ) : (
                                            <table className="w-full border-collapse text-[7px]">
                                                <thead>
                                                    <tr className="border-b border-rose-200">
                                                        {exportColumns.map(k => (
                                                            <th key={k} className="px-1.5 py-1 text-left font-black text-rose-600 uppercase">{COLUMN_DEFS.find(c => c.key === k)?.exportKey || k}</th>
                                                        ))}
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {exportPreviewData.rows.slice(0, 3).map((row, ri) => (
                                                        <tr key={ri} className="border-b border-rose-100">
                                                            {exportColumns.map(k => (
                                                                <td key={k} className="px-1.5 py-1">{row[COLUMN_DEFS.find(c => c.key === k)?.exportKey || k] ?? ''}</td>
                                                            ))}
                                                        </tr>
                                                    ))}
                                                </tbody>
                            ...
                            {previewFormat === 'ics' && (
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <span className="text-[9px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">iCal Preview</span>
