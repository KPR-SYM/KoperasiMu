...
import { Checkbox, PrivacyMask } from "@shared/components"
import InlineCell from "./InlineCell"
import PeriodContextTooltip from "./PeriodContextTooltip"

const PeriodDesktopRow = memo(function PeriodDesktopRow({
    year,
    orderedCols,
    selectedIds,
    toggleSelect,
    ...
    const toggleMenu = useCallback((e) => {
        if (openMenuId === year.id) {
            ...
            setMenuPos({
                ...
                bottom: showUp ? window.innerHeight - rect.top + 4 : undefined,
                left: rect.right - 176,
            })
            setOpenMenuId(year.id)
        }
    }, [openMenuId, year.id, closeMenu])
...
const PeriodMobileRow = memo(function PeriodMobileRow({
    year,
    selectedIds,
    toggleSelect,
    isPrivacyMode,
    ...
    }, [mobileMenuId, year.id, closeMobileMenu])
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3">
                <Checkbox checked={isSelected} onChange={() => toggleSelect(year.id)} small />
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center text-sm font-black shadow-sm relative transition-transform shrink-0 ${isPinned ? "bg-amber-500/10 text-amber-600" : year.is_active ? "bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-accent)]/10 text-[var(--color-primary)]" : "bg-[var(--color-surface-alt)] text-[var(--color-text-muted)]"}`}>
                ...
                <div className="flex-1 min-w-0 flex items-center gap-3" onClick={() => handleOpenReadOnlyDetail(year)}>
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap shrink-0">
                        ...
                        {year.is_active ? (
                            <span className="text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md border border-emerald-500/20 bg-emerald-500/10 text-emerald-600 whitespace-nowrap">Aktif</span>
                        ) : (
                            <span className="text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-md border border-[var(--color-border)] bg-[var(--color-surface-alt)] text-[var(--color-text-muted)] whitespace-nowrap">Nonaktif</span>
                        )}
                        {year.is_locked && (
            ...
            <div className="mt-3 pt-3 border-t border-[var(--color-border)]/40 flex items-center justify-between gap-2 pl-[26px]">
                ...
                <div className="flex items-center gap-0.5 shrink-0">
                    <button onClick={() => onTogglePin?.(year.id)} title={isPinned ? "Lepas pin" : "Pin ke atas"} className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all active:scale-95 ${isPinned ? "text-amber-500 bg-amber-500/10 border-amber-500/20" : "text-[var(--color-text-muted)] hover:text-amber-500 hover:bg-amber-500/10 border-[var(--color-border)]"}`}>
                        <PushPin weight={isPinned ? "fill" : "regular"} className="text-sm" />
                    </button>
                    <button onClick={() => handleOpenReadOnlyDetail(year)} title="Lihat Detail" className="w-9 h-9 rounded-xl flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:bg-[var(--color-primary)]/10 border border-[var(--color-border)] transition-all active:scale-95">
                        <MagnifyingGlass className="text-sm" />
                    </button>
                    {canEdit && !year.is_locked && (
                        <button onClick={() => handleEdit(year)} title="Edit" className="w-9 h-9 rounded-xl flex items-center justify-center text-[var(--color-text-muted)] hover:text-blue-500 hover:bg-blue-500/10 border border-[var(--color-border)] transition-all active:scale-95">
                            <Pencil className="text-sm" />
                        </button>
                    )}
                    <div className="relative">
                        <button onClick={(e) => toggleMobileMenu(e)} title="Lainnya" className="w-9 h-9 rounded-xl flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-alt)] border border-[var(--color-border)] transition-all active:scale-95">
                            <DotsThree weight="bold" />
                        </button>
                        {mobileMenuId === year.id && createPortal(
                            <div ref={mobileMenuRef} style={{ position: "fixed", top: mobileMenuPos.top, right: mobileMenuPos.right, zIndex: 9999 }} className="w-48 py-1.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-xl">
                                {canEdit && (
...
export { PeriodDesktopRow, PeriodMobileRow, COL_LABELS, renderColHeader, renderColCell }
