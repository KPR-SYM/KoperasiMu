...
const SHORTCUTS = [
    ...
    { keys: ["V"], label: "Ganti tampilan", icon: FadersHorizontal, action: "toggleView" },
    { keys: ["?"], label: "Tampilkan shortcut ini", icon: Keyboard, action: null },
    { section: "Aksi" },
    { keys: ["N"], label: "Tambah periode baru", icon: Plus, action: "add" },
    { keys: [MOD, "I"], label: "Import CSV / Excel", icon: Upload, action: "import" },
    { keys: [MOD, "E"], label: "Export Data", icon: Download, action: "export" },
    { keys: [MOD, "G"], label: "Generate Tahun Baru", icon: ArrowClockwise, action: "generate" },
    { keys: ["E"], label: "Edit periode terpilih", icon: Pencil, action: "edit", needsSelection: true },
    { keys: [MOD, "D"], label: "Duplikasi periode", icon: Copy, action: "duplicate", needsSelection: true },
    ...
    { keys: ["⌫"], label: "Arsipkan terpilih", icon: Trash, action: "bulkDelete", needsSelection: true },
    { keys: ["L"], label: "Kunci / buka kunci", icon: Lock, action: "toggleLock", needsSelection: true },
    { keys: ["H"], label: "Riwayat periode", icon: ClockCounterClockwise, action: "history", needsSelection: true },
    { section: "Mode" },
    { keys: [MOD, "P"], label: "Toggle mode privasi", icon: Eye, action: "privacy" },
    { keys: [MOD, "Z"], label: "Undo", icon: ArrowUUpLeft, action: "undo" },
    { keys: [MOD, "Y"], label: "Redo", icon: ArrowUUpRight, action: "redo" },
];
...
const PeriodsShortcutMenu = memo(function PeriodsShortcutMenu({
    ...
    const container = getPortalContainer("portal-periods-shortcut-menu");
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-0.5">
                                                {item.keys.map((k, ki) => (
                                                    <kbd
                                                        key={ki}
                                                        className="min-w-[20px] h-5 px-1 rounded-md bg-[var(--color-surface-alt)] border border-[var(--color-border)]/60 text-[10px] font-semibold text-[var(--color-text-muted)] font-mono inline-flex items-center justify-center"
                                                    >
                                                        {k}
                                                    </kbd>
                                                ))}
