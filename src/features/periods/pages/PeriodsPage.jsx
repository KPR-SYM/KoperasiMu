...
const LazyPeriodExportModal = React.lazy(() => import('@features/periods/components/PeriodExportModal'))
const LazyPeriodImportModal = React.lazy(() => import('@features/periods/components/PeriodImportModal'))
...
export default function PeriodsPage() {
    ...
    // Columns
    const defaultCols = { period: true, semester: true, duration: true, status: true }
    ...
    const [exportColumns, setExportColumns] = useState(['academic_year', 'semester', 'start_date', 'end_date', 'is_active', 'is_locked'])
    ...
    const handleSubmit = async (formData, setFormErrors) => {
        ...
        try {
            ...
            if (overlap) {
                ...
                setFormErrors({ endDate: `Periode tumpang tindih dengan ${dupe.academic_year} ${dupe.semester}` })
                setSubmitting(false)
                return
            }
            ...
            if (err?.code === '23505') {
                ...
                if (clash) {
                    setFormErrors?.({ endDate: `Periode bentrok dengan ${clash.academic_year} (${clash.semester}) · ${formatDate(clash.start_date)}–${formatDate(clash.end_date)}` })
                } else {
    ...
    const handleSetActive = async (item) => {
        ...
        setSubmitting(true)
        try {
            const { error: e1 } = await supabase.from('periods').update({ is_active: false }).neq('id', item.id).select()
            if (e1) throw e1
    ...
    const handleExportPDF = async (filename, options = {}) => {
        ...
        try {
            ...
            doc.setFontSize(8)
            doc.text(`Tanggal: ${new Date().toLocaleDateString('id-ID')}  |  Total: ${allRows.length} periode  |  Scope: ${exportScope === 'filtered' ? 'Funnel Aktif' : exportScope === 'selected' ? 'Dipilih' : 'Semua'}`, 14, 18)
    ...
    const buildImportPreview = async (rawRows, mapping) => {
        ...
        try {
            ...
            preview.forEach((row, i) => {
                ...
                if (row.academic_year && row.semester && years.some(y => y.academic_year === row.academic_year && y.semester === row.semester)) {
    ...
    const handleBulkFix = (colKey, value) => {
        setImportPreview(prev => prev.map(r => ({ ...r, [colKey]: value, _hasError: colKey === 'academic_year' ? !value : r._hasError })))
        if (colKey === 'academic_year' && value) setImportIssues(prev => prev.filter(iss => !iss.messages.includes('Tahun Pelajaran tidak boleh kosong')))
    ...
    const handleCommitImport = async () => {
        ...
        try {
            ...
            for (let i = 0; i < validRows.length; i += CHUNK) {
                ...
                const { error } = await supabase.from('periods').insert(chunk); if (error) throw error
    ...
    }, [years, searchQuery, filterSemester, filterStatus, filterLock, filterTimeStatus, sortBy])
            ...
            <div className="space-y-4 max-w-[1800px] mx-auto min-h-screen relative">
                ...
                {!canEdit && (
                    <div className="px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center gap-2">
                        ...
                        <p className="text-[11px] font-bold text-rose-600">Mode Read-only — Pen tahun pelajaran dinonaktifkan oleh administrator.</p>
                    </div>
                )}
                ...
                <StatsCarousel count={STAT_CARD_COUNT} cols={4}>
                    {[
                        { icon: StackSimple, label: 'Total Periode', value: stats.total, borderColor: 'border-t-blue-500', iconBg: 'bg-blue-500/10 text-blue-500' },
                        { icon: CheckCircle, label: 'Status Aktif', value: stats.active, borderColor: 'border-t-emerald-500', iconBg: 'bg-emerald-500/10 text-emerald-500' },
                        { icon: GraduationCap, label: 'Smt. Ganjil', value: stats.ganjil, borderColor: 'border-t-indigo-500', iconBg: 'bg-indigo-500/10 text-indigo-500' },
                        { icon: GraduationCap, label: 'Smt. Genap', value: stats.genap, borderColor: 'border-t-purple-500', iconBg: 'bg-purple-500/10 text-purple-500' },
                    ].map((s, i) => (
                        <StatCard
                            key={i}
                            icon={s.icon}
                                                        ...
                                                        <div className="absolute right-3 top-1/2 -translate-y-1/2">
                                                            ...
                                                            {isColMenuOpen && createPortal(
                                                                <div ref={colMenuPortalRef} className={`absolute z-[9999] w-48 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] shadow-2xl shadow-black/10 p-2 space-y-0.5 animate-in fade-in zoom-in-95 ${colMenuPos.showUp ? 'slide-in-from-bottom-2' : 'slide-in-from-top-2'}`}
                                                                    ...
                                                                    {[{ key: 'period', label: 'Tahun Pelajaran' }, { key: 'semester', label: 'Semester' }, { key: 'duration', label: 'Pelaksanaan' }, { key: 'status', label: 'Status' }].map(({ key, label }) => (
                                                        ...
                                                        <tr key={year.id} className={`border-t border-[var(--color-border)] transition-colors group/row ${isSelected ? 'bg-[var(--color-primary)]/5' : 'hover:bg-[var(--color-surface-alt)]/40'}`}>
                                                            ...
                                                            <td className="px-6 py-4">
                                                                <div className="flex items-center justify-center gap-1">
                                                                    {canEdit && (
                                                                        <button onClick={() => handleEdit(year)} title="Pen" className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-blue-500 hover:bg-blue-500/10 transition-all text-sm"><Pencil /></button>
                                                                    )}
                                                                    <button onClick={() => handleOpenHistory(year)} title="Riwayat" className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-purple-500 hover:bg-purple-500/10 transition-all text-sm"><ClockCounterClockwise /></button>
                                                                    {canEdit && (
                                                                        <button onClick={() => { setItemToDelete(year); setIsDeleteModalOpen(true) }} title="Hapus" className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-red-500 hover:bg-red-500/10 transition-all text-sm"><Trash /></button>
                                                                    )}
                                                                </div>
                                                            </td>
                                                        </tr>
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-start justify-between gap-2">
                                                                <div className="min-w-0 flex-1" onClick={() => handleOpenReadOnlyDetail(year)}>
                                                                    <button className="font-extrabold text-sm text-[var(--color-text)] hover:text-[var(--color-primary)] text-left truncate block w-full">{year.academic_year}</button>
                                                            ...
                                                            <div className="mt-3 flex items-center gap-2">
                                                                ...
                                                                <button onClick={() => handleOpenHistory(year)} className="flex-1 h-9 rounded-xl bg-indigo-500/10 text-indigo-600 border border-indigo-500/20 flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest active:scale-95 transition-all"><ClockCounterClockwise className="text-xs" /> Riwayat</button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                            ...
                            <div className="space-y-4 pb-2">
                                <div className="p-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-alt)]/30 flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        <h4 className="w-5 h-5 font-black text-[var(--color-text)] leading-tight truncate">{readOnlyDetailItem.academic_year}</h4>
                                    ...
                                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black shrink-0 ${readOnlyDetailItem.is_active ? 'bg-[var(--color-primary)] text-white shadow-md' : 'bg-[var(--color-surface)] border border-[var(--color-border)] text-[var(--color-text-muted)]'}`}>{readOnlyDetailItem.academic_year?.slice(2, 4) || '??'}</div>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]">
                                        ...
                                        <div className="w-3 h-3 font-bold">{formatDate(readOnlyDetailItem.start_date)} — {formatDate(readOnlyDetailItem.end_date)}</div>
                                    </div>
                                    <div className="p-4 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]">
                                        <p className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text-muted)] mb-2">Durasi</p>
                                        <div className="w-3 h-3 font-black">{getDuration(readOnlyDetailItem.start_date, readOnlyDetailItem.end_date)}</div>
                                    </div>
                                </div>
                                <button onClick={() => { setIsReadOnlyDetailOpen(false); handleOpenHistory(readOnlyDetailItem) }} className="w-full h-12 rounded-xl bg-indigo-500/10 text-indigo-600 font-black text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 border border-indigo-500/20"><ClockCounterClockwise />Riwayat Perubahan</button>
