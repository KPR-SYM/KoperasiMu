...
const DATASETS = {
    plotting: {
        ...
        columns: [
            { key: 'name', labelKey: 'dorms.plotting.thSantri', icon: User },
            { key: 'class', labelKey: 'dorms.plotting.class', icon: Buildings },
            { key: 'gender', labelKey: 'dorms.plotting.gender', icon: GenderIntersex },
            { key: 'room', labelKey: 'dorms.plotting.dorm', icon: House },
            { key: 'building', labelKey: 'dorms.plotting.building', icon: Warehouse },
            { key: 'status', labelKey: 'dorms.plotting.thStatus', icon: CheckSquare }
        ],
        presets: [
    ...
    cleanliness: {
        ...
        columns: [
            { key: 'room', labelKey: 'dorms.plotting.dorm', icon: House },
            { key: 'score', labelKey: 'dorms.export.scoreAverage', icon: Star },
            { key: 'rating', labelKey: 'dorms.export.predicate', icon: SlidersHorizontal },
            { key: 'aspect_kerapian', labelKey: 'dorms.export.aspectKerapian', icon: CheckCircle },
            { key: 'aspect_kebersihan', labelKey: 'dorms.export.aspectKebersihan', icon: CheckCircle },
            { key: 'aspect_keharuman', labelKey: 'dorms.export.aspectKeharuman', icon: CheckCircle },
            { key: 'date', labelKey: 'dorms.cleanliness.dateLabel', icon: Calendar },
            { key: 'notes', labelKey: 'dorms.musyrif.notes', icon: FileText }
        ],
        presets: [
    ...
    inventory: {
        ...
        columns: [
            { key: 'dorm', labelKey: 'dorms.plotting.dorm', icon: House },
            { key: 'item', labelKey: 'dorms.export.itemName', icon: ClipboardText },
            { key: 'total', labelKey: 'dorms.export.totalItem', icon: Hash },
            { key: 'good', labelKey: 'dorms.export.conditionGood', icon: CheckCircle },
            { key: 'damaged', labelKey: 'dorms.export.conditionDamaged', icon: Warning },
            { key: 'notes', labelKey: 'dorms.musyrif.notes', icon: FileText },
            { key: 'last_checked', labelKey: 'dorms.export.lastChecked', icon: Calendar }
        ],
        presets: [
...
export default function DormsExportModal({
        ...
        return rawList.map(item => {
            ...
            exportColumns.forEach(key => {
                ...
                if (dataset === 'plotting') {
                    ...
                    if (key === 'class') row[headerLabel] = item.classes?.name || 'â€”'
                    if (key === 'gender') row[headerLabel] = item.gender === 'putra' ? t('dorms.export.genderPutra') : item.gender === 'putri' ? t('dorms.export.genderPutri') : 'â€”'
                    if (key === 'room') row[headerLabel] = roomName || 'â€”'
                    if (key === 'building') row[headerLabel] = dormObj?.building || 'â€”'
                    if (key === 'status') row[headerLabel] = roomName ? t('dorms.export.statusAssigned') : t('dorms.export.statusUnassigned')
                } else if (dataset === 'cleanliness') {
                    if (key === 'room') row[headerLabel] = item.room || 'â€”'
                    if (key === 'score') row[headerLabel] = item.score ?? 0
                    if (key === 'rating') row[headerLabel] = item.rating ? t(`dorms.cleanliness.predicate`).replace('{rating}', item.rating) : 'â€”'
                    ...
                    if (key === 'date') row[headerLabel] = item.date || 'â€”'
                    if (key === 'notes') row[headerLabel] = item.notes || 'â€”'
                } else if (dataset === 'inventory') {
                    if (key === 'dorm') row[headerLabel] = item.dorm_id || 'â€”'
                    if (key === 'item') row[headerLabel] = item.item_name || 'â€”'
                    ...
                    if (key === 'notes') row[headerLabel] = item.notes || 'â€”'
                    if (key === 'last_checked') row[headerLabel] = item.last_checked_at ? new Date(item.last_checked_at).toLocaleDateString(language === 'id' ? 'id-ID' : language === 'ar' ? 'ar-EG' : 'en-US') : 'â€”'
                }
            })
            return row
        })
    ...
    const columnButtons = useMemo(() => currentDatasetDef.columns.map(({ key, labelKey, icon: Icon }) => {
                ...
                <div className={`space-y-4 pb-2 transition-all duration-500 ${exporting ? 'blur-sm grayscale-[0.5] opacity-50 pointer-events-none' : ''}`}>
                    ...
                    {dataset === 'plotting' && (
                        <div className="space-y-3">
                            ...
                            <div className="grid grid-cols-4 gap-2.5">
                                {[
                                    { val: 'all', label: t('dorms.export.scopeAll'), desc: `${tNum(students.length)}${t('dorms.suffixSantri')}`, icon: Users },
                                    { val: 'assigned', label: t('dorms.export.scopeAssigned'), desc: `${tNum(students.filter(s => s.metadata?.kamar).length)}${t('dorms.suffixSantri')}`, icon: House },
                                    { val: 'unassigned', label: t('dorms.export.scopeUnassigned'), desc: `${tNum(students.filter(s => !s.metadata?.kamar).length)}${t('dorms.suffixSantri')}`, icon: SlidersHorizontal },
                                    { val: 'selected', label: t('dorms.export.scopeSelected'), desc: `${tNum(selectedIds.length)}${t('dorms.suffixSantri')}`, icon: CheckCircle, disabled: selectedIds.length === 0 }
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            >
                                <GearSix className={advancedOpen ? 'animate-spin-slow' : ''} />
                        ...
                        {advancedOpen && (
                            <div className="p-4 rounded-2xl bg-[var(--color-surface-alt)] border border-[var(--color-border)] grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-200">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex gap-1 bg-[var(--color-surface)] p-1 rounded-lg border border-[var(--color-border)]">
                                        {[
                                            { v: 'landscape', l: t('dorms.export.landscape'), icon: ArrowsLeftRight },
                                            { v: 'portrait', l: t('dorms.export.portrait'), icon: ArrowsDownUp }
                                        ].map(opt => (
                                            <button
                                                key={opt.v}
                                                type="button"
