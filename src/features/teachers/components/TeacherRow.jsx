...
const TeacherRow = memo(({
    ...
    return (
        ...
        `}>
            ...
            {visibleCols.join && <td className="px-6 py-4 w-3 h-3 text-[var(--color-text-muted)]">{teacher.join_date ? new Date(teacher.join_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}</td>}
            <td className="px-6 py-4">
                <div className="flex items-center justify-center gap-1">
                    ...
                    {handleEdit && (
                        <button onClick={() => handleEdit(teacher)} title="Pen" className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-blue-500 hover:bg-blue-500/10 transition-all text-sm">
                            <Pencil />
                        </button>
                    )}
