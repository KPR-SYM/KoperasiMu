...
const EditableCell = React.memo(({ rowIdx, colKey, value, subjectsList, importEditCell, setImportEditCell, handleImportCellEdit, STATUS_CONFIG }) => {
    ...
    const renderDropdown = (content) => {
        ...
        return createPortal(
            <div className="fixed z-[9999]" style={{ bottom: (window.innerHeight - coords.anchorTop) + 8, left: coords.left, minWidth: Math.max(coords.width, colKey === 'subject' ? 220 : 140) }}>
    ...
    if (isEditing) {
        if (colKey === 'subject') {
            const filteredSubjects = subjectsList.filter(s => s.toLowerCase().includes(searchTerm.toLowerCase()))
            return (
                    ...
                    {renderDropdown(
                        <>
                            <div className="p-2 border-b border-[var(--color-border)] bg-[var(--color-surface-alt)]/50">
                                <input autoFocus className="w-full bg-transparent text-[10px] font-bold outline-none placeholder:font-normal placeholder:opacity-30" placeholder="Cari mata pelajaran..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} onKeyDown={e => { if (e.key === 'Escape') setImportEditCell(null); if (e.key === 'Enter' && filteredSubjects.length > 0) { handleImportCellEdit(rowIdx, colKey, filteredSubjects[0]); setImportEditCell(null) } }} />
                            </div>
                            <div className="max-h-[160px] overflow-auto py-1 scrollbar-none">
                                {filteredSubjects.length > 0 ? filteredSubjects.map(s => (
                                    <button key={s} className="w-full px-4 py-2 text-left text-[10px] font-bold hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] transition-colors flex items-center justify-between group" onClick={() => { handleImportCellEdit(rowIdx, colKey, s); setImportEditCell(null) }}>
                                        <span className="truncate">{s}</span>
                                        {value === s && <Check className="w-2 h-2" />}
                                    </button>
                                )) : (
                                    <div className="px-3 py-3 text-[9px] text-[var(--color-text-muted)] italic text-center">Data tidak ditemukan</div>
                                )}
                            </div>
                            <button className="p-2 text-[8px] font-black uppercase tracking-widest text-[var(--color-text-muted)] hover:text-red-500 transition-colors border-t border-[var(--color-border)]" onClick={() => setImportEditCell(null)}>
                                Batal
                            </button>
                        </>
                    )}
        ...
        if (colKey === 'gender') {
            const options = [{ id: 'L', name: 'Laki-laki' }, { id: 'P', name: 'Perempuan' }]
            return (
                <div ref={cellRef} className="relative">
                    ...
                    {renderDropdown(
                        <div className="py-1">
                            {options.map(opt => (
                                <button key={opt.id} className="w-full px-4 py-2.5 text-left text-[10px] font-bold hover:bg-[var(--color-primary)]/10 hover:text-[var(--color-primary)] transition-colors flex items-center justify-between" onClick={() => { handleImportCellEdit(rowIdx, colKey, opt.id); setImportEditCell(null) }}>
                                    <span>{opt.name}</span>
    ...
    return (
        <div className={`group cursor-pointer hover:bg-[var(--color-primary)]/5 px-1.5 py-0.5 -mx-1.5 rounded-md transition-all flex items-center ${isCentered ? 'justify-center' : 'justify-between'} gap-2 min-h-[20px] ${isEmpty ? 'text-red-500/40 italic font-normal' : ''}`} onClick={() => setImportEditCell({ row: rowIdx, col: colKey })}>
...
export default function TeacherImportPanel(props) {
    const {
        isOpen, onClose, importing, importStep, setImportStep, importPreview,
        importFileName, importFileInputRef, importDragOver, setImportDragOver,
        processImportFile, subjectsList, handleDownloadTemplate,
        ...
        handleCommitImport, hasImportBlockingErrors, importReadyRows,
        handleImportCellEdit, importEditCell, setImportEditCell,
    ...
    useEffect(() => {
        ...
        if (showSubjectsDropdown) document.addEventListener('mousedown', handleClickOutside)
        ...
        <div className="flex flex-col min-h-[calc(100vh-3.5rem)]">
            <div className="px-5 pt-5 pb-3 shrink-0">
                ...
                <div>
                    ...
                    <p className="text-[var(--color-text-muted)] text-[10px] mt-0.5 font-medium">Unggah data guru secara masal dari file Excel/CSV. Sistem akan memvalidasi data secara otomatis.</p>
                </div>
            </div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-3 bg-[var(--color-surface-alt)]/50 rounded-2xl border border-[var(--color-border)] shadow-sm">
                                            <div className="flex items-center gap-6">
                                                <div className="flex items-center gap-2.5">
                                                    <div className="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-600"><Book className="w-3 h-3" /></div>
                                                    <div className="flex flex-col"><span className="text-[10px] font-black uppercase tracking-wider text-[var(--color-text)]">Data Referensi</span><span className="text-[8px] font-bold text-emerald-600">Guru & Mapel</span></div>
                                                </div>

                                                <div className="relative" ref={subjectsDropdownRef}>
                                                    <button onClick={() => setShowSubjectsDropdown(!showSubjectsDropdown)} className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border transition-all duration-300 ${showSubjectsDropdown ? 'bg-emerald-500 text-white border-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-[var(--color-surface)] border-[var(--color-border)] text-[var(--color-text)] hover:border-emerald-500/50 hover:bg-emerald-500/5'}`}>
                                                        ...
                                                        <CaretDown className={`w-2 h-2 transition-transform duration-300 ${showSubjectsDropdown ? 'rotate-180' : ''}`} />
                                                    </button>
                                                    ...
                                                    {showSubjectsDropdown && (
                                                        <div className="absolute top-full left-0 mt-2 w-[320px] bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] shadow-2xl z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
                                                            <div className="p-3 border-b border-[var(--color-border)]/50"><span className="text-[9px] font-black text-[var(--color-text-muted)] uppercase tracking-widest">Daftar Mapel (Copy & Paste):</span></div>
                                                            <div className="p-2 max-h-[140px] overflow-y-auto custom-scrollbar">
                                                                <div className="grid grid-cols-2 gap-1.5">
                                                                    {subjectsList.map(s => (
                                                                        <span key={s} className="px-2 py-1.5 rounded-lg bg-[var(--color-surface-alt)] border border-[var(--color-border)] text-[9px] font-bold text-[var(--color-text)] hover:border-emerald-500/30 transition-colors truncate">{s}</span>
                                                                    ))}
                                                                </div>
                                                            </div>
                                                            <div className="p-3 bg-[var(--color-surface-alt)]/30 rounded-b-2xl border-t border-[var(--color-border)]/50"><p className="text-[8px] text-[var(--color-text-muted)] italic leading-tight">* Pastikan penulisan di Excel persis seperti di atas.</p></div>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>

                                            <button onClick={handleDownloadTemplate} className="shrink-0 h-9 px-4 rounded-xl bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-emerald-600 hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-emerald-500/20"><DownloadSimple /> Download Template</button>
                                        </div>
                                        ...
                                        <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border)] overflow-hidden shadow-sm flex flex-col">
                                            <div className="px-4 py-2 bg-[var(--color-surface-alt)] border-b border-[var(--color-border)] flex items-center justify-between">
                                                <div className="flex items-center gap-2"><List className="text-[var(--color-primary)] w-3 h-3" /><span className="text-[10px] font-black uppercase tracking-wider text-[var(--color-text-muted)]">Visualisasi Struktur Kolom Excel</span></div>
                                                    ...
                                                    <tbody>
                                                        {SAMPLE_ROWS.map((row, rIdx) => (
                                                            <tr key={rIdx}>
                                                                ...
                                                                {row.map((cell, cIdx) => (
                                                                    <td key={cIdx} className="px-2 py-1 border-r border-b border-[var(--color-border)] bg-[var(--color-surface)]/40 overflow-hidden"><span className="text-[9px] font-medium text-[var(--color-text)] opacity-70 truncate block" title={cell}>{cell}</span></td>
                                                                ))}
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                            ...
                                            <div className="px-4 py-1.5 bg-[var(--color-surface)] border-t border-[var(--color-border)] flex items-center justify-between">
                                                <p className="text-[8px] text-[var(--color-text-muted)] font-medium italic opacity-60">* Gunakan judul kolom yang mendekati nama di atas untuk pencocokan otomatis.</p>
                                                ...
                                                <div key={sys.key} className={`p-2.5 rounded-xl border transition-all ${mapped ? 'bg-emerald-500/4 border-emerald-500/20' : 'bg-[var(--color-surface-alt)]/50 border-[var(--color-border)]'}`}>
                                                    <div className="flex items-center justify-between gap-3">
                                                        <div className="flex flex-col w-[130px] shrink-0">
                                                            <span className="text-[10px] font-black text-[var(--color-text)] flex items-center gap-1">{sys.label}{['name'].includes(sys.key) && <span className="text-red-500 text-[9px]">*</span>}</span>
                                                            <span className="text-[8px] font-bold text-[var(--color-text-muted)] opacity-50 uppercase tracking-tight">Sistem</span>
                                                        </div>
                                                        <div className="flex items-center gap-1.5 opacity-30"><ArrowRight className={`w-2 h-2 ${mapped ? 'text-emerald-500 opacity-100' : ''}`} /></div>
                                                        <div className="flex-1 min-w-0 relative">
                                                            <Select small value={mapped || ''} onChange={(val) => setImportColumnMapping(v => ({ ...v, [sys.key]: val }))} options={importFileHeaders.map(h => ({ id: h, name: h }))} placeholder="-- Lewati Kolom --" extraOption={{ id: '', name: '-- Lewati Kolom --' }} status={mapped ? 'success' : 'normal'} searchable={importFileHeaders.length > 5} />
                                                        </div>
                                                    </div>
                                                </div>
                                            )
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex flex-wrap items-center justify-between gap-3 p-2 rounded-2xl bg-[var(--color-surface-alt)]/50 border border-[var(--color-border)] shadow-sm">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <button onClick={() => setImportSkipDupes(!importSkipDupes)} className={`flex items-center gap-2 h-8 px-3 rounded-xl border text-[10px] font-black uppercase tracking-tight transition-all ${importSkipDupes ? 'bg-violet-500 text-white border-violet-500 shadow-md shadow-violet-500/20' : 'bg-[var(--color-surface)] text-[var(--color-text-muted)] border-[var(--color-border)] hover:border-violet-500/40 hover:text-violet-600'}`}><Copy className="w-3 h-3" /><span>{importSkipDupes ? 'Lewati Duplikat' : 'Ikutkan Duplikat'}</span></button>
                                                            ...
                                                            {importPreview.map((r, originalIdx) => ({ ...r, originalIdx })).filter(r => !filterIssuesOnly || (r._hasError || r._isDupe || r._hasWarn)).slice(0, 300).map((r) => {
                                                                ...
                                                                return (
                                                                    <tr key={i} className={`hover:bg-[var(--color-surface-alt)]/40 transition-colors border-b border-[var(--color-border)]/30 last:border-0 ${isError ? 'bg-red-500/3' : isDupe ? 'bg-violet-500/3' : ''}`}>
                                                                        <td className="px-2 py-0.5 font-bold text-[var(--color-text)] text-[10px] truncate"><EditableCell rowIdx={i} colKey="name" value={r.name} importPreview={importPreview} subjectsList={subjectsList} importEditCell={importEditCell} setImportEditCell={setImportEditCell} handleImportCellEdit={handleImportCellEdit} STATUS_CONFIG={STATUS_CONFIG} /></td>
                                                                        <td className="px-2 py-0.5 text-[var(--color-text-muted)] font-bold text-[10px] truncate"><EditableCell rowIdx={i} colKey="subject" value={r.subject} importPreview={importPreview} subjectsList={subjectsList} importEditCell={importEditCell} setImportEditCell={setImportEditCell} handleImportCellEdit={handleImportCellEdit} STATUS_CONFIG={STATUS_CONFIG} /></td>
                                                                        <td className="px-2 py-0.5 text-center text-[var(--color-text-muted)] font-bold text-[10px]"><EditableCell rowIdx={i} colKey="gender" value={r.gender} importPreview={importPreview} subjectsList={subjectsList} importEditCell={importEditCell} setImportEditCell={setImportEditCell} handleImportCellEdit={handleImportCellEdit} STATUS_CONFIG={STATUS_CONFIG} /></td>
                                                                        <td className="px-2 py-0.5 text-[var(--color-text-muted)] font-bold text-[10px] truncate"><EditableCell rowIdx={i} colKey="phone" value={r.phone} importPreview={importPreview} subjectsList={subjectsList} importEditCell={importEditCell} setImportEditCell={setImportEditCell} handleImportCellEdit={handleImportCellEdit} STATUS_CONFIG={STATUS_CONFIG} /></td>
                                                                        <td className="px-2 py-0.5 text-center text-[var(--color-text-muted)] font-bold text-[10px]"><EditableCell rowIdx={i} colKey="status" value={r.status} importPreview={importPreview} subjectsList={subjectsList} importEditCell={importEditCell} setImportEditCell={setImportEditCell} handleImportCellEdit={handleImportCellEdit} STATUS_CONFIG={STATUS_CONFIG} /></td>
                                                                        <td className="px-2 py-1">
                                                                            <div className="flex items-center justify-center gap-2">
                                                                                {isError ? <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-500/15 text-red-600 animate-pulse"><WarningCircle className="w-3 h-3" /></span>
                                                                                    : isDupe ? <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-violet-500/15 text-violet-600"><Copy className="w-3 h-3" /></span>
                                                                                        : isWarn ? <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-amber-500/15 text-amber-600"><Warning className="w-3 h-3" /></span>
                                                                                            : <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-green-500/15 text-green-600"><CheckCircle className="w-3 h-3" /></span>}
                        ...
                        <div className="flex items-center justify-between px-5 py-3.5 border-t border-[var(--color-border)] bg-[var(--color-surface-alt)]/30">
                            <div className="flex items-center gap-3">
                                {importStep > 1 && !importing && (
                                    <button onClick={() => setImportStep(v => v - 1)} className="h-[38px] px-4 rounded-[10px] border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text-muted)] text-[12.5px] font-bold hover:bg-[var(--color-surface-alt)] transition-all flex items-center gap-2">
                                        <ArrowLeft className="w-3.5 h-3.5" /> Kembali
                                    </button>
                                )}
                                {!showSidebar && (
                                    <button onClick={() => setShowSidebar(true)} className="h-[38px] px-3 rounded-[10px] border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-text-muted)] text-[10px] font-bold hover:bg-[var(--color-surface-alt)] transition-all flex items-center gap-1.5">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {importStep === 2 && (
                                    <button onClick={async () => { setImportStep(3); setImportLoading(true); await buildImportPreview(importRawData, importColumnMapping); setImportLoading(false) }} disabled={!importColumnMapping.name} className="h-[38px] px-5 rounded-[10px] bg-[var(--color-primary)] text-white text-[12.5px] font-bold hover:brightness-110 disabled:opacity-40 transition-all flex items-center gap-2 shadow-md shadow-[var(--color-primary)]/20">
