...
import { Warning, Archive, ArrowDown, ArrowsDownUp, Suitcase, Calendar, Checks, CheckCircle, CheckSquare, CaretLeft, CaretRight, CaretDoubleLeft, CaretDoubleRight, DownloadSimple, Eye, EyeSlash, FileArrowDown, FileText, FileArrowUp, Fingerprint, IdentificationCard, Keyboard, List, Spinner, Envelope, MapPin, GenderMale, MegaphoneSimple, ChatCircle, Pencil, Phone, PresentationChart, MagnifyingGlass, ShieldCheck, Trash, UploadSimple, X, ArrowCounterClockwise, GenderFemale, Plus, SlidersHorizontal, UserCheck, SortDescending } from '@phosphor-icons/react'
            ...
            addToast(
                newPinned ? (
                    <span className="flex items-center gap-1.5">
                        <Pin className="text-amber-400 rotate-[-45deg] w-3 h-3" />
            ...
            <div className="space-y-4 max-w-[1800px] mx-auto min-h-screen relative">
                ...
                {!canEdit && (
                    <div className="px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center gap-2">
                        ...
                        <p className="text-[11px] font-bold text-rose-600">Mode Read-only — Pen data guru dinonaktifkan oleh administrator.</p>
                    </div>
                )}
                ...
                {selectedIds.length > 0 && (
                    <div className="mb-4 px-4 py-3 rounded-2xl bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 flex items-center justify-between gap-3 flex-wrap">
                        <p className="w-4 h-4 font-black text-[var(--color-primary)]">{selectedIds.length} guru dipilih</p>
                        <div className="flex items-center gap-2 flex-wrap">
                            <button onClick={startBulkWA} className="h-8 px-3 rounded-xl bg-green-500/10 text-green-600 text-[10px] font-black uppercase tracking-wide hover:bg-green-500 hover:text-white transition-all flex items-center gap-1.5"><ChatCircle />WA Massal</button>
                            <button onClick={() => setIsBulkModalOpen(true)} className="h-8 px-3 rounded-xl bg-amber-500/10 text-amber-600 text-[10px] font-black uppercase tracking-wide hover:bg-amber-500 hover:text-white transition-all flex items-center gap-1.5"><Archive />Arsip</button>
                            <button onClick={() => setSelectedIds([])} className="h-8 px-3 rounded-xl bg-[var(--color-surface-alt)] text-[var(--color-text-muted)] text-[10px] font-black uppercase tracking-wide transition-all flex items-center gap-1.5"><X />Batal</button>
                        </div>
                    </div>
                )}
                ...
                <StatsCarousel count={STAT_CARD_COUNT} cols={4}>
                    {[
                        { icon: ArrowCounterClockwise, label: 'Total', value: stats.total, borderColor: 'border-t-[var(--color-primary)]', iconBg: 'bg-gradient-to-br from-[var(--color-primary)]/10 to-[var(--color-accent)]/10 text-[var(--color-primary)]', onClick: () => { setFilterType(''); setPage(1) } },
                        { icon: CheckCircle, label: 'Aktif', value: stats.active, borderColor: 'border-t-emerald-500', iconBg: 'bg-emerald-500/10 text-emerald-500', onClick: () => { setFilterStatus('active'); setPage(1) } },
                        { icon: ArrowCounterClockwise, label: 'Guru', value: stats.guru, borderColor: 'border-t-indigo-500', iconBg: 'bg-indigo-500/10 text-indigo-500', onClick: () => { setFilterType('guru'); setPage(1) } },
                        { icon: Suitcase, label: 'Karyawan', value: stats.karyawan, borderColor: 'border-t-blue-500', iconBg: 'bg-blue-500/10 text-blue-500', onClick: () => { setFilterType('karyawan'); setPage(1) } },
                    ].map((s, i) => (
                        <StatCard
                            key={i}
                            icon={s.icon}
                    ...
                    <div className="flex overflow-x-auto scrollbar-hide gap-2 mb-6 animate-in fade-in slide-in-from-top-1 duration-500 pb-1">
                        {insights.map((ins) => (
                            ...
                            >
                                ...
                                >
                                    <ins.icon className={`text-[10px] ${ins.active ? 'text-white' : ins.iconColor}`} />
                                </div>
                ...
                <div className="glass rounded-[1.5rem] mb-4 border border-[var(--color-border)] overflow-hidden">
                    ...
                    <div className="flex items-center gap-2 p-2.5 lg:p-3">
                        {/* ShieldCheck Bar - Dynamic & Responsive */}
                        ...
                        <div className="hidden lg:flex flex-1 items-center gap-2 overflow-x-auto scrollbar-hide py-0.5 min-w-0 pr-8 h-full [mask-image:linear-gradient(to_right,black_calc(100%-32px),transparent)]">
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                                {[
                                    ...
                                    { id: 'P', label: 'Putri', icon: X, activeCls: 'bg-pink-500 border-pink-500' },
                                ].map((g) => (
                                    <button
                                        key={g.id}
                    ...
                    {showAdvFilter && (
                        <div className="border-t border-[var(--color-border)] p-3.5 bg-[var(--color-surface-alt)]/60 backdrop-blur-md animate-in fade-in slide-in-from-top-2">
                            ...
                            <div className="grid grid-cols-1 xl:grid-cols-[1fr_auto] gap-4">
                                ...
                                {/* Secondary GridFour: Quick Filters */}
                                <div>
                                    ...
                                    <div className="flex gap-1.5 overflow-x-auto pb-1">
                                        {[
                                            ...
                                            { label: 'Nonaktif', icon: X, active: filterStatus === 'inactive', onClick: () => { setFilterStatus('inactive'); setPage(1) } },
                ...
                <Modal isOpen={isBulkWAOpen} onClose={() => setIsBulkWAOpen(false)} title="WA Massal Guru" size="sm">
                    <div className="space-y-4">
                        ...
                        <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)] w-3 h-3 text-[var(--color-text-muted)] font-medium">
