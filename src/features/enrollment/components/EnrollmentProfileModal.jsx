...
function InfoRow({ icon, label, value, color = '' }) {
    ...
    return (
        <div className="flex items-start gap-3 py-2 border-b border-[var(--color-border)]/30 last:border-0">
            <icon className={`w-3 h-3 mt-0.5 ${color || 'text-[var(--color-text-muted)] opacity-40'}`} />
                ...
                canEdit ? (
                    <div className="flex items-center gap-2 w-full justify-between sm:justify-end">
                        ...
                        <div className="flex items-center gap-1 bg-[var(--color-surface-alt)]/50 p-1 rounded-xl border border-[var(--color-border)] mr-auto">
                            <button onClick={() => onEdit(enrollment)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface)] shadow-sm transition-all" title="Pen Data">
            ...
            <div className="px-1">
                ...
                <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-5 mb-5 relative overflow-hidden">
                    ...
                    <div className="flex items-start gap-4 relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className="w-5 h-5 font-bold text-[var(--color-text)] leading-tight truncate">{name}</h3>
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                                <span className="text-[9px] font-bold text-[var(--color-text-muted)] tracking-wider">{registration_number}</span>
                                {gender === 'L' ? <GenderMale className={`w-2.5 h-2.5 text-blue-400`} /> : <GenderFemale className={`w-2.5 h-2.5 text-pink-400`} />}
                                ...
                                <div key={ps.key} className="flex items-center flex-1">
                                    <div className={`flex-1 flex flex-col items-center gap-1 transition-all ${isRejected ? 'opacity-30' : ''}`}>
                                        <div className={`w-6 h-6 rounded-lg flex items-center justify-center text-[9px] font-black ${isActive
                                            ...
                                            {isDone ? <CheckCircle className="w-2 h-2" /> : i + 1}
                                        </div>
                    ...
                    {isConverted && (
                        <div className="mt-3 flex items-center gap-2.5 px-3 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
                            <GraduationCap className="text-indigo-600 w-4 h-4 animate-pulse" />
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4">
                        <div className="flex items-center gap-2.5 pt-1 mb-4">
                            ...
                            <User className="text-indigo-500 w-3 h-3 opacity-70" />
                        ...
                        <InfoRow icon={Calendar} label="Tempat, Tanggal Lahir" value={`${birth_place || '-'}, ${birth_date || '-'}`} />
                        <InfoRow icon={GraduationCap} label="NISN" value={nisn} />
                        <InfoRow icon={Buildings} label="Asal Sekolah" value={school_origin} />
                        {previous_pesantren && <InfoRow icon={Buildings} label="Asal Pesantren" value={previous_pesantren} color="text-emerald-500" />}
                        <InfoRow icon={ClipboardText} label="Program" value={getProgramLabel(program)} color="text-indigo-500" />
                    </div>
                    ...
                    <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4">
                        <div className="flex items-center gap-2.5 pt-1 mb-4">
                            ...
                            <Book className="text-emerald-500 w-3 h-3 opacity-70" />
                    ...
                    <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4">
                        <div className="flex items-center gap-2.5 pt-1 mb-4">
                            ...
                            <Users className="text-purple-500 w-3 h-3 opacity-70" />
                            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text)]">Data Keluarga</span>
                            <div className="h-[1px] flex-1 bg-gradient-to-r from-[var(--color-border)] to-transparent opacity-40" />
                        </div>
                        <InfoRow icon={User} label="Ayah" value={`${father_name || '-'}${father_occupation ? ` · ${father_occupation}` : ''}`} />
                        {father_phone && (
                            <div className="flex items-center justify-between group">
                                <div className="flex-1 min-w-0"><InfoRow icon={Phone} label="HP Ayah" value={father_phone} color="text-emerald-500" /></div>
                        ...
                        <InfoRow icon={User} label="Ibu" value={`${mother_name || '-'}${mother_occupation ? ` · ${mother_occupation}` : ''}`} />
                        {mother_phone && (
                            <div className="flex items-center justify-between group">
                                <div className="flex-1 min-w-0"><InfoRow icon={Phone} label="HP Ibu" value={mother_phone} color="text-emerald-500" /></div>
                    ...
                    <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4">
                        <div className="flex items-center justify-between pt-1 mb-4">
                            <div className="flex items-center gap-2.5">
                                <div className="w-1 h-4 bg-indigo-500 rounded-full" />
                                <Money className="text-indigo-500 w-3 h-3 opacity-70" />
                            ...
                            {canEdit && (
                                ...
                                >
                                    <Pen className="w-2 h-2" /> Kelola Keuangan
                                </button>
                            )}
                        </div>
                                ...
                                return (
                                    <div key={item.key} className="p-3 border border-[var(--color-border)]/65 rounded-xl bg-[var(--color-surface-alt)]/25 flex flex-col justify-between min-h-[72px]">
                                        <div>
                                            ...
                                            <p className="w-3 h-3 font-black text-[var(--color-text)] mt-0.5 tabular-nums">
                    ...
                    <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4">
                        <div className="flex items-center gap-2.5 pt-1 mb-4">
                            ...
                            <ClipboardText className="text-amber-500 w-3 h-3 opacity-70" />
                            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--color-text)]">Info Tambahan</span>
                            <div className="h-[1px] flex-1 bg-gradient-to-r from-[var(--color-border)] to-transparent opacity-40" />
                        </div>
                        <InfoRow icon={MapPin} label="Alamat" value={address} color="text-indigo-500" />
                        {phone ? (
                            <div className="flex items-center justify-between group">
                                <div className="flex-1 min-w-0"><InfoRow icon={Phone} label="HP Utama" value={phone} color="text-emerald-500" /></div>
                            ...
                            <InfoRow icon={Phone} label="HP Utama" value={phone} color="text-emerald-500" />
                        )}
                        <InfoRow icon={Heart} label="Riwayat Kesehatan" value={health_notes} color="text-rose-500" />
                        <InfoRow icon={TShirt} label="Ukuran Seragam" value={uniform_size} />
                        <InfoRow icon={Calendar} label="Tanggal Daftar" value={formatEnrollmentDate(created_at)} />
                    ...
                    {(status === 'diterima' || status === 'daftar_ulang' || enrollment.metadata?.orientation) && (
                        <div className="bg-[var(--color-surface)] border border-indigo-500/10 bg-indigo-500/[0.01] shadow-sm rounded-2xl p-4 mt-4">
                            <div className="flex items-center justify-between pt-1 mb-3">
                                <div className="flex items-center gap-2.5">
                                    <div className="w-1 h-4 bg-indigo-500 rounded-full" />
                                    <Calendar className="text-indigo-500 w-3 h-3 opacity-70" />
                                ...
                                {canEdit && !isEditingOrientation && (
                                    <button
                                        ...
                                        className="px-2.5 py-1 rounded-lg border border-indigo-500/20 text-[9px] font-bold text-indigo-600 bg-indigo-500/5 hover:bg-indigo-500/10 transition-colors flex items-center gap-1"
                                    >
                                        <Pen className="w-2 h-2" /> {enrollment.metadata?.orientation ? 'Pen Jadwal' : 'Atur Jadwal'}
                                    </button>
                                )}
                            </div>
                                ...
                                <div className="space-y-2.5">
                                    <div className="grid grid-cols-2 gap-4 bg-[var(--color-surface-alt)]/40 p-3 rounded-xl border border-[var(--color-border)]/50">
                                        <div>
                                            ...
                                            <p className="w-3 h-3 font-black text-[var(--color-text)] mt-1">
                                        ...
                                        <div>
                                            ...
                                            <p className="w-3 h-3 font-black text-[var(--color-text)] mt-1 truncate" title={enrollment.metadata.orientation.location}>
                    ...
                    {enrollment.interview && (
                        <div className="bg-[var(--color-surface)] border border-indigo-500/10 bg-indigo-500/[0.01] shadow-sm rounded-2xl p-4 mt-4">
                            <div className="flex items-center justify-between pt-1 mb-4">
                                <div className="flex items-center gap-2.5">
                                    <div className="w-1 h-4 bg-indigo-500 rounded-full" />
                                    <Star className="text-indigo-500 w-3 h-3 opacity-70" />
                                    ...
                                    return (
                                        <div key={idx} className="bg-[var(--color-surface-alt)]/30 border border-[var(--color-border)]/50 rounded-xl p-2.5">
                                            ...
                                            <div className="flex items-baseline gap-1 mt-1">
                                                <span className="w-4 h-4 font-black text-[var(--color-text)]">{item.val || '-'}</span>
                    ...
                    <div className="bg-[var(--color-surface)] border border-amber-500/20 bg-amber-500/[0.02] shadow-sm rounded-2xl p-4 mt-4">
                        <div className="flex items-center justify-between pt-1 mb-3">
                            ...
                            {canEdit && !isEditingNotes && notes && (
                                ...
                                >
                                    <Pen className="w-2 h-2" /> Pen Catatan
                                </button>
                            )}
                        </div>
                    ...
                    {/* Activity Log / Status ClockCounterClockwise */}
                    {enrollment.metadata?.history?.length > 0 && (
                        <div className="bg-[var(--color-surface)] shadow-sm border border-[var(--color-border)] rounded-2xl p-4 mt-4">
                            <div className="flex items-center gap-2.5 pt-1 mb-4">
                                ...
                                <ClockCounterClockwise className="text-sky-500 w-3 h-3 opacity-70" />
