...
import { ArrowLeft, ArrowRight, Book, CheckCircle, CaretRight, ClipboardText, Eye, GenderMale, Pen, Buildings, Trash, UserPlus, GenderFemale, XCircle } from '@phosphor-icons/react'
        ...
        <tr className={`group transition-colors duration-150 ${isSelected ? 'bg-[var(--color-primary)]/5' : 'hover:bg-[var(--color-surface-alt)]/50'}`}>
                ...
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {gender === 'L' ? <GenderMale className="w-2 h-2 text-blue-400" /> : <GenderFemale className="w-2 h-2 text-pink-400" />}
                            {missingCount === 0 ? (
            ...
            <td className="py-3 px-3">
                <div className="flex items-center gap-1">
                    ...
                    {canEdit && (
                        <>
                            <button onClick={() => onEdit(enrollment)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--color-text-muted)] hover:text-blue-500 hover:bg-blue-500/10 transition-all text-sm" title="Pen">
                                <Pen />
                            </button>
        ...
        >
            ...
            <div className="px-4 pb-3 grid grid-cols-2 gap-2">
                <div className="flex items-center gap-2 text-[10px]">
                    <Buildings className="text-[var(--color-text-muted)] opacity-40 w-3 h-3" />
                    <span className="text-[var(--color-text-muted)] font-medium truncate">{school_origin || '-'}</span>
                </div>
                <div className="flex items-center gap-2 text-[10px]">
                    <Book className="text-[var(--color-text-muted)] opacity-40 w-3 h-3" />
                ...
                <div className="flex items-center gap-1 px-3 py-2.5 border-t border-[var(--color-border)]/50 bg-[var(--color-surface-alt)]/20">
                    ...
                    {status !== 'mendaftar' && (
                        <>
                            ...
                            >
                                <ArrowLeft className="w-2 h-2" /> Kembali
                            </button>
                        </>
                    )}
                    {/* FastForward action on Mobile */}
