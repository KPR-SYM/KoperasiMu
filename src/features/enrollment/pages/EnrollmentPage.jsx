...
import EnrollmentFormModal from '@features/enrollment/components/EnrollmentFormModal'
import EnrollmentProfileModal from '@features/enrollment/components/EnrollmentProfileModal'
import EnrollmentWaveModal from '@features/enrollment/components/EnrollmentWaveModal'
import EnrollmentSearch from '@features/enrollment/components/EnrollmentSearch'
import EnrollmentArchiveModal from '@features/enrollment/components/EnrollmentArchiveModal'
import EnrollmentConvertModal from '@features/enrollment/components/EnrollmentConvertModal'
import EnrollmentImportModal from '@features/enrollment/components/EnrollmentImportModal'
import EnrollmentExportModal from '@features/enrollment/components/EnrollmentExportModal'
import EnrollmentStatsModal from '@features/enrollment/components/EnrollmentStatsModal'
import EnrollmentAssessmentModal from '@features/enrollment/components/EnrollmentAssessmentModal'
import EnrollmentPaymentModal from '@features/enrollment/components/EnrollmentPaymentModal'
import { useEnrollmentImportExport } from '@features/enrollment/hooks/useEnrollmentImportExport'
import { STATUS_CONFIG, PROGRAM_OPTIONS, REQUIRED_DOCUMENTS } from '@features/enrollment/utils/enrollmentConstants'
        ...
        <DashboardLayout title="PSB / Enrollment" subtitle="Penerimaan Santri Baru">
            <div className="space-y-4 max-w-[1800px] mx-auto relative">
                        ...
                        {isHeaderMenuOpen && headerMenuRect && createPortal(
                            <>
                                ...
                                >
                                    ...
                                    >
                                        <div className="w-8 h-8 rounded-lg bg-indigo-500/10 text-indigo-500 flex items-center justify-center group-hover:scale-110 transition-transform">
                                            {sendingReminders ? <Spinner className="w-3 h-3 animate-spin" /> : <Bell className="w-3 h-3" />}
                                        </div>
                                        <div className="text-left">
                ...
                <StatsCarousel count={5} cols={5}>
                    <StatCard icon={WavesLadder} label="Total Pendaftar" value={globalStats.total} color="sky" />
                    <StatCard icon={Spinner} label="Menunggu" value={globalStats.mendaftar + globalStats.verifikasi} color="amber" />
                    <StatCard icon={Book} label="Tahap Tes" value={globalStats.tes} color="indigo" />
                    <StatCard icon={CheckCircle} label="Diterima" value={globalStats.diterima} color="emerald" subValue={globalStats.quota ? `Kuota: ${globalStats.quotaLeft} sisa` : undefined} />
                    ...
                    <div className="flex items-center gap-2 p-2.5 lg:p-3">
                        ...
                        <div className="hidden lg:flex flex-1 items-center gap-2 overflow-x-auto scrollbar-hide py-0.5 min-w-0 pr-8 [mask-image:linear-gradient(to_right,black_calc(100%-32px),transparent)]">
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                                {[
                                    { id: '', label: 'Semua', icon: WavesLadder },
                                    { id: 'mendaftar', label: 'Mendaftar', icon: FileArrowDown },
                                    { id: 'verifikasi', label: 'Verifikasi', icon: Spinner },
                                    { id: 'tes', label: 'Tes', icon: Book },
                                    { id: 'diterima', label: 'Diterima', icon: CheckCircle },
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                                {[
                                    { id: 'L', label: 'Putra', icon: ChartPie, cls: 'bg-blue-500 border-blue-500 hover:bg-blue-600' },
                                    { id: 'P', label: 'Putri', icon: WavesLadder, cls: 'bg-pink-500 border-pink-500 hover:bg-pink-600' }
                                ].map(g => (
                                    <button
                                        key={g.id}
                            ...
                            <div className="flex flex-wrap gap-2">
                                {searchQuery && (
                                    <button onClick={() => setSearchQuery('')} className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)]/40 text-[10px] font-black text-[var(--color-text)]">
                                        ...
                                        <span className="w-5 h-5 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] group-hover:text-red-500 transition-colors"><X className="w-3 h-3" /></span>
                                    </button>
                                )}
                                {filterStatus && (
                                    <button onClick={() => setFilterStatus('')} className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-[var(--color-primary)]/20 bg-[var(--color-primary)]/5 text-[10px] font-black text-[var(--color-primary)]">
                                        ...
                                        <span className="w-5 h-5 rounded-lg bg-white/70 dark:bg-[var(--color-surface)] border border-[var(--color-primary)]/20 flex items-center justify-center opacity-70 group-hover:opacity-100"><X className="w-3 h-3" /></span>
                                    </button>
                                )}
                                {filterWave && (
                                ...
                                {filterGender && (
                                    <button onClick={() => setFilterGender('')} className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface-alt)]/40 text-[10px] font-black text-[var(--color-text)]">
                                        {filterGender === 'L' ? <GenderMale className="opacity-70" /> : <GenderFemale className="opacity-70" />} {filterGender === 'L' ? 'Putra' : 'Putri'}
                                        <span className="w-5 h-5 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)] flex items-center justify-center text-[var(--color-text-muted)] group-hover:text-red-500"><X className="w-3 h-3" /></span>
                                    </button>
                                )}
                                {filterProgram && (
                                    <button onClick={() => setFilterProgram('')} className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-indigo-500/20 bg-indigo-500/10 text-[10px] font-black text-indigo-600">
                                        ...
                                        <span className="w-5 h-5 rounded-lg bg-white/70 dark:bg-[var(--color-surface)] border border-indigo-500/20 flex items-center justify-center opacity-70 group-hover:opacity-100"><X className="w-3 h-3" /></span>
                                    </button>
                                )}
                                {(filterDateFrom || filterDateTo) && (
                                    <button onClick={() => { setFilterDateFrom(''); setFilterDateTo(''); }} className="group inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-teal-500/20 bg-teal-500/10 text-[10px] font-black text-teal-600">
                                        ...
                                        <span className="w-5 h-5 rounded-lg bg-white/70 dark:bg-[var(--color-surface)] border border-teal-500/20 flex items-center justify-center opacity-70 group-hover:opacity-100"><X className="w-3 h-3" /></span>
                                    </button>
                                )}
                                <button onClick={resetAllFilters} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-red-500/20 bg-red-500/5 text-[10px] font-black text-red-600">
                                    <ArrowCounterClockwise className="w-3 h-3" /> Reset semua
                                </button>
                            </div>
                        </div>
                    )}
                                    ...
                                    <tr><td colSpan={8} className="py-16 text-center px-4">
                                        {activeFilterCount > 0 || searchQuery ? (
                                            ...
                                            <EmptyState
                                                ...
                                                action={
                                                    <button onClick={handleAdd} className="px-5 py-2.5 rounded-xl bg-[var(--color-primary)] hover:bg-[var(--color-primary)]/90 text-white w-3 h-3 font-black uppercase tracking-widest shadow-lg shadow-[var(--color-primary)]/20 transition-all active:scale-95 flex items-center justify-center gap-1.5 mx-auto">
                                                        <UserPlus className="w-3 h-3" /> Tambah Pendaftar
                                                    </button>
                                                }
                                            />
                                        )}
                        ...
                        {loading ? (
                            ...
                            activeFilterCount > 0 || searchQuery ? (
                                ...
                                <EmptyState
                                    ...
                                    action={
                                        <button onClick={handleAdd} className="px-4 py-2 rounded-xl bg-[var(--color-primary)] text-white w-3 h-3 font-black uppercase tracking-widest">
                                            Tambah Pendaftar
                                        </button>
                                    }
                                    variant="dashed"
            ...
            <Modal
                ...
                icon={validationModal.type === 'docs' ? ClipboardText : Spinner}
            ...
            <Modal isOpen={activeModal === 'delete'} onClose={closeModal} title="Arsipkan Pendaftar" maxWidth="max-w-sm">
                <div className="px-6 py-4 text-center">
                    <div className="w-14 h-14 mx-auto mb-4 rounded-2xl bg-amber-500/10 flex items-center justify-center"><Archive className="text-amber-500 text-xl" /></div>
                    <p className="w-4 h-4 font-bold text-[var(--color-text)] mb-1">Arsipkan "{enrollmentToDelete?.name}"?</p>
                ...
                >
                    <div className="space-y-4 py-1">
                        ...
                        <div className="space-y-1.5">
                            {[
                                { key: 'mendaftar', label: 'Mendaftar', desc: 'Pendaftaran baru masuk, menunggu berkas', activeColor: 'bg-sky-500/10 text-sky-500', iconColor: 'text-sky-500', barColor: 'bg-sky-500', icon: FileArrowDown },
                                { key: 'verifikasi', label: 'Verifikasi', desc: 'Berkas dan data sedang divalidasi', activeColor: 'bg-amber-500/10 text-amber-600', iconColor: 'text-amber-500', barColor: 'bg-amber-500', icon: Spinner },
                                { key: 'tes', label: 'Tahap Tes', desc: 'Ujian seleksi lisan & baca Quran', activeColor: 'bg-purple-500/10 text-purple-600', iconColor: 'text-purple-500', barColor: 'bg-purple-500', icon: Book },
                                { key: 'diterima', label: 'Diterima', desc: 'Lolos seleksi & resmi bergabung', activeColor: 'bg-emerald-500/10 text-emerald-600', iconColor: 'text-emerald-500', barColor: 'bg-emerald-500', icon: CheckCircle },
                                { key: 'ditolak', label: 'Ditolak', desc: 'Tidak memenuhi kriteria penerimaan', activeColor: 'bg-rose-500/10 text-rose-600', iconColor: 'text-rose-500', barColor: 'bg-rose-500', icon: XCircle }
                            ].map(opt => (
                                <button
                                    key={opt.key}
                                    type="button"
