...
const TABLE_CONFIG = [
    ...
    { key: 'audit_logs', label: 'Audit Logs', icon: Database, color: 'text-slate-500', bg: 'bg-slate-500/10', softDelete: false, optional: true, dateCol: 'created_at' },
    { key: 'school_settings', label: 'Pengaturan Sekolah', icon: FloppyDisk, color: 'text-cyan-500', bg: 'bg-cyan-500/10', softDelete: false, optional: true, dateCol: null },
]
...
const SEVERITY_STYLE = {
    error: { color: 'text-red-500', bg: 'bg-red-500/10', border: 'border-red-500/25', icon: WarningCircle },
    warning: { color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/25', icon: Warning },
    info: { color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/25', icon: Info },
    ok: { color: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500/25', icon: CheckCircle },
}
        ...
        return (
            <div className="flex items-center gap-3 px-5 py-3.5 opacity-40">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${table.bg}`}>
                    <table.icon className={`text-[11px] ${table.color}`} />
                </div>
        ...
        <div className={`transition-colors ${isExpanded ? 'bg-[var(--color-primary)]/[0.02]' : 'hover:bg-[var(--color-surface-alt)]/50'}`}>
            ...
            >
                ...
                <div className="hidden md:flex items-center justify-center">
                    {isExpanded ? <CaretUp className={`w-3 h-3 text-[var(--color-primary)]`} /> : <CaretDown className={`w-3 h-3 text-[var(--color-border)]`} />}
                </div>
            </div>
                ...
                <div className="px-5 pb-4 border-t border-[var(--color-border)]/50">
                    <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)]">
                            ...
                            <p className="w-5 h-5 font-black text-[var(--color-text)] tabular-nums">{fmtNumber(active ?? total)}</p>
                        </div>
                        {table.softDelete && (
                            <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)]">
                                <p className="text-[8px] font-black uppercase tracking-widest text-[var(--color-text-muted)] mb-1">Soft Deleted</p>
                                <p className={`w-5 h-5 font-black tabular-nums ${deleted > 0 ? 'text-amber-500' : 'text-[var(--color-text)]'}`}>
                                    {fmtNumber(deleted ?? 0)}
                                </p>
                            </div>
                        )}
                        <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)]">
                            ...
                            <p className="w-5 h-5 font-black text-[var(--color-primary)] tabular-nums">{fmtNumber(total)}</p>
                        </div>
                        <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)]">
        ...
        <div className={`rounded-xl border transition-all ${style.border} ${isOk ? 'bg-transparent' : isExpanded ? style.bg : 'hover:' + style.bg}`}>
            ...
            >
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${style.bg}`}>
                    {isOk ? <CheckCircle className={`w-3 h-3 ${style.color}`} /> : <style.icon className={`text-[11px] ${style.color}`} />}
                </div>
                <div className="flex-1 min-w-0">
                ...
                {canExpand && (
                    <div className="flex items-center gap-2 shrink-0">
                        {check.canRepair && !isOk && (
                            ...
                            >
                                {repairing ? <Spinner className="animate-spin" /> : <Trash />}
                                <span>{repairing ? 'Membersihkan...' : 'Bersihkan'}</span>
                            </button>
                        )}
                        {isExpanded ? <CaretUp className={`w-3 h-3 ${style.color}`} /> : <CaretDown className={`w-3 h-3 text-[var(--color-text-muted)]`} />}
                    </div>
                )}
            </div>
        ...
        <DashboardLayout title="Database Health">
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
                <div className="w-20 h-20 rounded-3xl bg-red-500/10 flex items-center justify-center">
                    <ShieldCheck className="text-3xl text-red-500" />
                </div>
                <div className="text-center">
                    <h2 className="text-xl font-black text-[var(--color-text)] mb-2">Akses Terbatas</h2>
                    <p className="text-[var(--color-text-muted)] w-4 h-4">
            ...
            <div className="p-4 md:p-6 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        >
                            <ArrowCounterClockwise className={autoRefresh ? 'animate-spin' : ''} style={{ animationDuration: '3s' }} />
                        ...
                        <button
                            ...
                            title="Refresh"
                        >
                            <ArrowClockwise className={`w-4 h-4 ${loadingTables ? 'animate-spin' : ''}`} />
                        </button>
                        <button
                ...
                <StatsCarousel count={4}>
                    {[
                        { label: 'Total Tabel', value: summaryStats.tableCount, icon: Database, color: 'text-cyan-500', bg: 'bg-cyan-500/10' },
                        { label: 'Total Rows', value: summaryStats.totalRows, icon: Lightning, color: 'text-[var(--color-primary)]', bg: 'bg-[var(--color-primary)]/10' },
                        { label: 'Integrity Score', value: integrityScore?.percentage != null ? `${integrityScore.percentage}%` : '—', icon: ShieldCheck, color: integrityScore?.percentage >= 90 ? 'text-emerald-500' : integrityScore?.percentage >= 70 ? 'text-amber-500' : 'text-red-500', bg: integrityScore?.percentage >= 90 ? 'bg-emerald-500/10' : integrityScore?.percentage >= 70 ? 'bg-amber-500/10' : 'bg-red-500/10' },
                        { label: 'Soft Deleted', value: summaryStats.totalDeleted, icon: Warning, color: 'text-amber-500', bg: 'bg-amber-500/10' },
                    ].map((s, i) => (
                ...
                <div className="glass rounded-[1.5rem] border border-[var(--color-border)] overflow-hidden">
                    <div className="flex items-center gap-2 p-3">
                        <div className="flex-1 relative">
                            <MagnifyingGlass className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)] w-4 h-4 pointer-events-none" />
                            <input
                    ...
                    {loadingTables ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-[var(--color-surface-alt)] flex items-center justify-center">
                                <Database className="text-2xl text-[var(--color-text-muted)] opacity-30" />
                            </div>
                            <div className="text-center">
                    ...
                    <div className="p-3 space-y-2">
                        {Object.keys(integrityResults).length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-12 gap-3">
                                <div className="w-14 h-14 rounded-2xl bg-[var(--color-surface-alt)] flex items-center justify-center">
                                    <Checks className="text-2xl text-[var(--color-text-muted)] opacity-30" />
                                </div>
                                <div className="text-center">
                ...
                <div className="p-3 rounded-xl bg-[var(--color-surface-alt)] border border-[var(--color-border)] flex items-start gap-2.5">
                    <Info className="text-[var(--color-text-muted)] w-3 h-3 mt-0.5 shrink-0" />
