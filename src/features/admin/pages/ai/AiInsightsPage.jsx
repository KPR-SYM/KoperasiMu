...
const getSecurityStatus = (query) => {
    ...
    if (isFlagged) return { label: 'Flagged', color: 'text-rose-500', bg: 'bg-rose-500/10', icon: Warning, flagged: true };
    return { label: 'Secure', color: 'text-emerald-500', bg: 'bg-emerald-500/10', icon: CheckCircle, flagged: false };
};
...
export default function AiInsightsPage() {
    ...
    const statsList = useMemo(() => [
        { label: 'Neural Logs', val: stats.total, icon: Terminal, color: 'text-indigo-600', bg: 'bg-indigo-600/10' },
        { label: 'Neural Threat Level', val: stats.threatLevel, icon: ShieldCheck, color: stats.threatLevel === 'LOW' ? 'text-emerald-500' : 'text-rose-500', bg: stats.threatLevel === 'LOW' ? 'bg-emerald-500/10' : 'bg-rose-500/10' },
        { label: 'Avg Efficiency', val: '99.4%', icon: Lightning, color: 'text-amber-500', bg: 'bg-amber-500/10' },
        { label: 'Total Sync Cost', val: `$ ${stats.cost}`, icon: Coins, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    ], [stats]);

    // ── Audit Export Utility (Poin 3) ──
    const exportAudit = (log) => {
        const headers = ["ID", "TextT", "Timestamp", "Query", "Response", "Security"];
    ...
    return (
        <DashboardLayout title="Neural Engine Insights">
            <div className="p-4 md:p-6 space-y-5 max-w-[1280px] mx-auto animate-in fade-in duration-500">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 animate-in slide-in-from-top-4 duration-500">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button onClick={fetchLogs}
                            ...
                            <ArrowClockwise className={`w-3 h-3 ${loading ? 'animate-spin' : ''}`} /> Refresh
                        </button>
                    </div>
                </div>

                {/* ── Rows 2: TerminalWindow Center Dashboard (Charts & Insights) ── */}
                ...
                {/* ── Rows 3: Unified Stats GridFour (Poin 4: Threat Level) ── */}
                        ...
                        {statsList.map((s, i) => (
                            <div key={i} className="flex-none w-2/3 sm:w-full snap-start sm:snap-align-none">
                                <div className="bg-[var(--color-surface)] border border-[var(--color-border)] p-4 rounded-[1.8rem] group/card hover:border-[var(--color-primary)]/30 transition-all flex items-center gap-4 h-[85px] relative overflow-hidden shadow-sm hover:shadow-md">
                                    ...
                                    {/* Neural Heartbeat Indicator (Top Right) */}
                ...
                {/* ── Rows 4: Neural TerminalWindow Toolbar (Poin 5: Advanced Filters) ── */}
                <div className="bg-[var(--color-surface)] rounded-[1.5rem] border border-[var(--color-border)] shadow-sm overflow-hidden mb-6 transition-all duration-300">
                    ...
                    {/* Row 1: MagnifyingGlass + Major Controls */}
                    <div className="flex items-center gap-2 p-3">
                        {/* MagnifyingGlass Cluster */}
                        <div className="relative flex-1 group min-w-0">
                            <MagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 w-3 h-3 text-[var(--color-text-muted)] opacity-50 group-focus-within:opacity-100" />
                            <input
                                value={searchInput}
                                onChange={e => setSearchInput(e.target.value)}
                                onKeyDown={(e) => e.key === 'Enter' && setSearchTerm(searchInput)}
                                placeholder="MagnifyingGlass neural logs (TextT & Press Enter)..."
                        ...
                        <div className="px-3 pb-4 border-t border-[var(--color-border)] animate-in slide-in-from-top-2 duration-300">
                            <div className="pt-4 flex flex-wrap items-start gap-6">
                                ...
                                <div className="flex flex-col gap-2">
                                    <label className="text-[8px] font-black uppercase tracking-widest text-[var(--color-text-muted)] ml-1 opacity-60">Neural Interaction TextT</label>
                                    <div className="flex flex-wrap gap-1.5">
                                        {[
                                            {id: 'all', label: 'Any Interaction', icon: Lightning},
                                            {id: 'chat', label: 'Neural Chat', icon: ChatCircle},
                                            {id: 'editor', label: 'Zen Editor', icon: PenNibStraight}
                                        ].map(t => (
                                            <button 
                                                key={t.id}
                                            ...
                                            >
                                                <t.icon className="text-[8px] opacity-70" />
                                                {t.label}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div className="flex flex-col gap-2 min-w-[200px]">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {[
                                            {id: 'all', label: 'All Records', icon: ControlTower},
                                            {id: 'secure', label: 'Safe Intel', icon: CheckCircle},
                                            {id: 'flagged', label: 'Risk Intel', icon: Warning}
                                        ].map(s => (
                                            <button 
                                                key={s.id}
                                            ...
                                            >
                                                <s.icon className="text-[8px] opacity-70" />
                                                {s.label}
                                            </button>
                                        ))}
                                    </div>
                        ...
                        <div className="px-3 pb-3 -mt-1 flex flex-wrap items-center gap-2 animate-in fade-in duration-300">
                            {searchTerm && (
                                <div className="inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-indigo-500/10 bg-indigo-500/5 text-[9px] font-black text-indigo-600">
                                    <MagnifyingGlass className="opacity-60" />
                ...
                {/* ── Rows 5: Content View (Switchable) ── */}
                {viewMode === 'list' ? (
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-3">
                                                            <div className={`w-8 h-8 rounded-lg ${sec.flagged ? 'bg-rose-500/10 text-rose-600' : 'bg-[var(--color-surface-alt)] text-[var(--color-text-muted)]'} flex items-center justify-center border border-[var(--color-border)] group-hover:bg-white transition-colors`}>
                                                                 {log.type === 'chat' ? <ChatCircle className="w-3 h-3" /> : <Terminal className="w-3 h-3" />}
                                                            </div>
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className={`text-[8px] font-black ${sec.color} uppercase tracking-widest flex items-center gap-1 opacity-70`}>
                                                                                                                                         <sec.icon className="text-[7px]" /> {sec.label}
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </td>
                ...
                {/* ── Rows 6: Standard Pagination (NewsListPage Consistency) ── */}
                    ...
                    <div className="px-6 py-4 bg-[var(--color-surface-alt)]/20 flex flex-col md:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2">
                            <div className="flex items-center bg-white dark:bg-black/10 rounded-xl border border-[var(--color-border)] p-0.5">
                                <button disabled={page === 0} onClick={() => setPage(0)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] hover:bg-[var(--color-surface-alt)] disabled:opacity-30"><CaretDoubleLeft /></button>
                                <button disabled={page === 0} onClick={() => setPage(p => p - 1)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] hover:bg-[var(--color-surface-alt)] disabled:opacity-30"><CaretLeft /></button>
                                <div className="px-3 text-[10px] font-black text-[var(--color-primary)]">{page + 1} / {totalPages}</div>
                                <button disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] hover:bg-[var(--color-surface-alt)] disabled:opacity-30"><CaretRight /></button>
                                <button disabled={page >= totalPages - 1} onClick={() => setPage(totalPages - 1)} className="w-8 h-8 rounded-lg flex items-center justify-center text-[10px] hover:bg-[var(--color-surface-alt)] disabled:opacity-30"><CaretDoubleRight /></button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {/* ── Rows 8: Detail Modal (Portal Optimized - Smooth asf) ── */}
                {
                                ...
                                <div className="md:w-56 shrink-0 bg-slate-50 dark:bg-slate-900/40 border-b md:border-b-0 md:border-r border-[var(--color-border)] p-4 md:p-5 flex md:flex-col items-center md:items-start justify-between md:justify-start gap-4">
                                    <div className="flex items-center gap-3 md:block">
                                        <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white text-md md:text-lg shadow-md shadow-indigo-500/20 md:mb-5">
                                            {selectedLog.type === 'chat' ? <ChatCircle /> : <Terminal />}
                                        </div>
                                            ...
                                            <div className="p-5 rounded-2xl rounded-tl-none bg-indigo-50/10 dark:bg-indigo-600/[0.02] border border-indigo-500/10 shadow-sm">
                                                ...
                                                <div className="mt-5 pt-3 border-t border-indigo-500/5 flex items-center justify-between opacity-50">
                                                    <div className="flex items-center gap-1.5">
                                                        ...
                                                        <span className="text-[9px] font-black uppercase tracking-widest text-emerald-500">SealCheck Log Record</span>
                                                    </div>
