...
export default function NewsEditorPage() {
        ...
        input.onchange = async () => {
            ...
            if (file) {
                ...
                if (!error) {
                    ...
                    // Sultan Feature: Auto-Alt TextT based on Title
                    setTimeout(() => {
                        const images = quill.root.querySelectorAll('img');
                        const lastImg = images[images.length - 1];
                        if (lastImg) lastImg.alt = form.title || 'Informasi Koperasi SenyumMu';
                    }, 100);
    ...
    // State for Editor TerminalWindow Strip (Replaces Modals)
    ...
    const setupSmartPaste = useCallback((editor) => {
        editor.root.addEventListener('paste', (e) => {
            ...
            const pastedText = clipboardData.getData('TextT');
    ...
    const ambienceOptions = [
        ...
        { id: 'rain', label: 'Rainy Day', icon: CloudRain, url: 'https://actions.google.com/sounds/v1/weather/rain_on_roof.ogg' },
        { id: 'cafe', label: 'Library', icon: Coffee, url: 'https://actions.google.com/sounds/v1/ambiences/coffee_shop.ogg' },
        { id: 'forest', label: 'Nature', icon: Leaf, url: 'https://assets.mixkit.co/active_storage/sfx/2431/2431-preview.mp3' } // Better Forest birds
    ];
    ...
    // ── Sultans Feature: Drag & Drop ImageSquare ──
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            const tooltips = {
                ...
                '.ql-video': 'Sisipkan VideoCamera Youtube',
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            if (imageFile) {
                ...
                if (uploadError) {
                    addToast('UploadSimple gagal: ' + uploadError.message, 'error')
                    setIsSaving(false)
                    return
                }
            ...
            // ── FloppyDisk Revision (Enterprise) ──
            if (isEdit) {
                try {
    ...
    return (
        <DashboardLayout title={isEdit ? 'Pen Informasi' : 'Buat Informasi Baru'}>
            <div className="w-full mx-auto space-y-5">
                    ...
                    <div className="flex items-center justify-between gap-3 min-w-0">
                        <div className="flex items-center gap-2 md:gap-4 min-w-0">
                            <button onClick={handleGoBack} className="shrink-0 w-9 h-9 md:w-10 md:h-10 flex items-center justify-center rounded-xl md:rounded-2xl bg-[var(--color-surface-alt)] border border-[var(--color-border)] text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors">
                                <CaretLeft className="w-3 h-3 md:w-4 h-4" />
                            </button>
                            <h1 className="w-5 h-5 md:text-2xl font-black font-heading tracking-tight text-[var(--color-text)] truncate">
                                {isEdit ? 'Pen Informasi' : 'Tulis Informasi Baru'}
                            </h1>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 md:gap-3 shrink-0">
                            ...
                            <button type="button" onClick={handleSave} disabled={isSaving || !form.title.trim() || !form.content.trim()}
                                ...
                                {isSaving ? <Spinner className="animate-spin" /> : <FloppyDisk />}
                ...
                <style>{`
                    ...
                    /* On DeviceTablet/Mobile allow wrap so dropdowns don't clip and tools stay accessible */
                    ...
                    /* Static VideoCamera Preview in Sidebar */
                ...
                <form id="news-form" onSubmit={handleSave} className="flex flex-col xl:flex-row gap-6">
                    ...
                    <div className="flex-1 space-y-6">
                        ...
                        <div className="flex items-center justify-between sm:justify-start gap-1 md:gap-2 p-1 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl w-full sm:w-fit shadow-sm overflow-hidden">
                            {[
                                ...
                                { id: 'settings', label: 'Pengaturan', icon: Funnel },
                                ...
                                <button key={t.id} type="button" onClick={() => setActiveTab(t.id)}
                                    ...
                                    <t.icon className="text-[10px] sm:w-3 h-3" />
                                    <span className="whitespace-nowrap">{t.label}</span>
                                </button>
                            ))}
                        </div>

                        {/* Contains Content / GearSix / SEO / Preview depending on active tab */}
                                ...
                                <div className="space-y-5 animate-in fade-in duration-300">
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)] opacity-80">
                                                    <Newspaper className="mr-2" />Judul Informasi <span className="text-rose-500">*</span>
                                                </label>
                                                <button
                                                    type="button"
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)] opacity-80">
                                                <TextAlignLeft className="mr-2" />Ringkasan Singkat
                                            </label>
                                            <div className="flex items-center gap-3">
                                                <button type="button"
                                                    onClick={() => {
                                    ...
                                    <div className="space-y-2">
                                        <div className="flex items-center justify-between mt-6 mb-2">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)] opacity-80">
                                                <BookOpenText className="mr-2" />Isi Informasi <span className="text-rose-500">*</span>
                                            </label>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="hidden xl:flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-500/5 border border-slate-500/10 text-[var(--color-text-muted)] text-[9px] font-black uppercase tracking-widest">
                                                    <Clock className="text-[var(--color-primary)]/50" />
                                                ...
                                                <button type="button" onClick={() => setIsZenMode(true)}
                                                    ...
                                                    title="Crosshair Zen Mode">
                                                    <ArrowsOutCardinal className="w-3 h-3 group-hover:scale-110 transition-transform" />
                    ...
                    .ql-editor.ql-blank::before {
                        ...
                        pointer-events: none !important;
                                            ...
                                            {/* TerminalWindow Strip UI (Enterprise Style) */}
                                            <div className={`absolute top-[42px] inset-x-0 z-[30] flex justify-center transition-all duration-300 ease-out ${commandStrip.open ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
                                                <div className="mx-4 mt-2 p-2 flex items-center gap-2 bg-[var(--color-surface)] border border-[var(--color-border)] shadow-2xl rounded-2xl min-w-[400px] max-w-[600px] w-full animate-in zoom-in-95">
                                                    <div className={`w-8 h-8 shrink-0 rounded-xl flex items-center justify-center text-[10px] shadow-sm ${commandStrip.type === 'link' ? 'bg-blue-500/10 text-blue-500' : 'bg-rose-500/10 text-rose-500'}`}>
                                                        {commandStrip.type === 'link' ? <Pen /> : <BookOpenText />}
                                                    </div>
                                                    ...
                                                    <input type="url" value={commandStrip.url}
                                                        ...
                                                        onKeyDown={e => {
                                                            if (e.key === 'Enter') {
                                                                ...
                                                                addToast(`${commandStrip.type === 'link' ? 'Tautan' : 'VideoCamera'} berhasil disisipkan`, 'success');
                                                    ...
                                                    <div className="flex items-center gap-1 shrink-0">
                                                        <button type="button" onClick={() => setCommandStrip(s => ({ ...s, open: false }))}
                                                            className="w-8 h-8 rounded-lg text-slate-400 hover:bg-slate-100 transition-all text-[10px]" title="Batal (Esc)"><ArrowCounterClockwise /></button>
                                            ...
                                            {/* Bubble Contextual List (Medium Style) */}
                                            {bubbleMenu.show && (
                                                <div className="absolute z-[40] flex items-center gap-1 p-1 bg-slate-900 shadow-xl rounded-xl border border-white/10 animate-in fade-in zoom-in-95 duration-200"
                                                    ...
                                                    <button type="button" onClick={() => quillRef.current.getEditor().format('bold', !quillRef.current.getEditor().getFormat().bold)}
                                                        className="w-8 h-8 rounded-lg text-white hover:bg-white/20 transition-all flex items-center justify-center text-xs"><TextB /></button>
                                                    <button type="button" onClick={() => quillRef.current.getEditor().format('italic', !quillRef.current.getEditor().getFormat().italic)}
                                                        className="w-8 h-8 rounded-lg text-white hover:bg-white/20 transition-all flex items-center justify-center text-xs"><TextItalic /></button>
                                                    <button type="button" onClick={() => handlers.link.call({ quill: quillRef.current.getEditor() })}
                                                        className="w-8 h-8 rounded-lg text-white hover:bg-white/20 transition-all flex items-center justify-center text-xs"><Link /></button>
                                            ...
                                            <div className="flex items-center justify-between p-4 bg-[var(--color-surface-alt)]/50 border-t border-[var(--color-border)] rounded-b-[1.5rem]">
                                                <div className="flex items-center gap-4">
                                                    <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-[var(--color-text-muted)] group cursor-help transition-all" title="Target: > 100 kata untuk SEO yang baik">
                                                        <TextAlignLeft className={`${(form.content?.replace(/<[^>]*>/g, '').length || 0) > 500 ? 'text-emerald-500' : 'text-[var(--color-primary)]/50'}`} />
                            ...
                            {activeTab === 'settings' && (
                                <div className="space-y-5 animate-in fade-in duration-300 max-w-3xl">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Kategori</label>
                                            <div className="relative group">
                                                ...
                                                />
                                                <Tag className="absolute right-4 top-1/2 -translate-y-1/2 w-3 h-3 text-[var(--color-text-muted)] opacity-40 pointer-events-none" />
                                        ...
                                        <div className="space-y-2">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Nama Penulis</label>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div className="space-y-2">
                                            <div className="flex items-center justify-between">
                                                <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Slug URL</label>
                                                <button type="button" onClick={refreshSlug} className="text-[9px] font-black uppercase tracking-widest text-[var(--color-primary)] hover:underline flex items-center gap-1">
                                                    <ArrowCounterClockwise className="w-2 h-2" /> Sinkron Judul
                                                </button>
                                            </div>
                                        ...
                                        {/* Jadwal */}
                                        <div className="space-y-2">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Jadwal Tayang</label>
                                    ...
                                    {/* Thumbnail UploadSimple */}
                                    <div className="space-y-2.5">
                                        <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Thumbnail Utama</label>
                                        ...
                                        <div className="flex items-center justify-between mt-2">
                                            {imageError ? (
                                                <p className="w-3 h-3 font-bold text-rose-500 flex items-center gap-1.5">
                                                    <Warning /> {imageError}
                                                </p>
                                            ) : <div className="flex items-center gap-2">
                                        ...
                                        <div className="space-y-2 pt-3 border-t border-[var(--color-border)]">
                                            <label className="w-3 h-3 font-black uppercase tracking-widest text-[var(--color-text-muted)]">Alt TextT Gambar</label>
                                        ...
                                        <div className="lg:w-[310px] p-4 rounded-[1.5rem] bg-indigo-50 border border-indigo-100 flex flex-col gap-4 shadow-sm relative overflow-hidden">
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="w-3 h-3 font-black text-indigo-950 uppercase tracking-tight leading-tight">
                                            ...
                                            <div className="space-y-2">
                                                <label className="text-[10px] font-black uppercase tracking-widest text-indigo-900/40">Crosshair Keyword</label>
                                        ...
                                        <div className="flex-1 p-4 rounded-[1.5rem] bg-[var(--color-surface-alt)]/30 border border-[var(--color-border)] flex flex-col gap-3.5">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <button type="button" onClick={generateAiSeo} disabled={isGeneratingSeo || !form.content}
                                                    ...
                                                    {isGeneratingSeo ? <Spinner className="animate-spin w-2 h-2" /> : <Sparkle className="w-2 h-2" />}
                                                ...
                                                {seoAnalysis.map((item, idx) => (
                                                    <div key={idx} className="flex items-start gap-2.5 text-[9px] sm:text-[10px]">
                                                        <div className={`w-3.5 h-3.5 shrink-0 mt-0.5 rounded-full flex items-center justify-center text-[6px] ${item.st === 'success' ? 'bg-emerald-500 text-white' : item.st === 'error' ? 'bg-rose-500 text-white' : 'bg-amber-500 text-white'}`}>
                                                            {item.st === 'success' ? <Check /> : item.st === 'error' ? <X /> : <Warning />}
                                                        </div>
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between border-b border-[var(--color-border)] pb-2">
                                                <p className="text-[11px] font-black uppercase tracking-widest text-[var(--color-text-muted)] flex items-center gap-2">
                                                    {seoView === 'search' ? <Globe className="opacity-50" /> : <ShareNetwork className="opacity-50" />}
                                                    {seoView === 'search' ? 'Google MagnifyingGlass Preview' : 'Social Media Card Preview'}
                                                </p>
                            ...
                            {activeTab === 'preview' && (
                                <div className="space-y-6 animate-in fade-in duration-300 max-w-3xl mx-auto">
                                    ...
                                    <div className="flex items-center gap-3 flex-wrap">
                                        <span className="px-4 py-1.5 rounded-full bg-[var(--color-primary)]/10 text-[var(--color-primary)] w-3 h-3 font-black uppercase tracking-widest">{form.tag}</span>
                                        <span className="text-[var(--color-text-muted)] w-4 h-4 font-bold flex items-center gap-2">
                                            <Clock className="opacity-50" /> {calculatedReadTime} menit baca
                                        </span>
                                    </div>
                                    <h2 className="text-3xl md:text-4xl font-black font-heading leading-tight text-[var(--color-text)]">
                                        {form.title || 'Judul Utama Informasi Akan Ditampilkan Dengan Gaya Tipografi TextH'}
                                    </h2>
                                    <div className="flex items-center gap-4 py-6 border-y border-[var(--color-border)]">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="w-4 h-4 font-black text-[var(--color-text)]">{form.display_name || 'Admin Platform'}</span>
                                            <span className="w-3 h-3 font-medium text-[var(--color-text-muted)]">{new Date().toLocaleDateString('id-ID', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
                                        </div>
                                    </div>
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-[var(--color-surface)] m-4 rounded-[1.5rem] border border-[var(--color-border)] shadow-sm">
                                <div className="space-y-6">
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap pb-2">
                                        ...
                                        <span className="text-[var(--color-text-muted)] text-[11px] font-bold uppercase tracking-widest"><Clock className="mr-1" /> {calculatedReadTime} mnt</span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-3 border-b border-[var(--color-border)] pb-5 pt-2 mt-4">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="w-4 h-4 font-black text-[var(--color-text)] leading-none mb-1">{form.display_name || 'Admin'}</span>
                    ...
                    {isZenMode && createPortal(
                        <div className="fixed inset-0 z-[99999] bg-[var(--color-surface)] flex flex-col animate-in fade-in duration-500 overflow-y-auto custom-scrollbar focus-zen-container">
                            ...
                            <div className={`sticky top-0 z-[30] bg-[var(--color-surface)]/80 backdrop-blur-xl px-4 sm:px-8 py-3 sm:py-4 flex items-center justify-between border-b border-[var(--color-border)] transition-all duration-700 ${isTyping ? 'opacity-10 pointer-events-none translate-y-[-10px]' : 'opacity-100'}`}>
                                <div className="flex items-center gap-3 sm:gap-4 overflow-hidden">
                                    <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-[var(--color-primary)]/10 flex items-center justify-center text-[var(--color-primary)] shrink-0">
                                        <ArrowSquareOut className="w-4 h-4 sm:w-5 h-5" />
                                    </div>
                                    <div className="truncate">
                                        <h2 className="w-5 h-5 sm:text-xl font-black text-[var(--color-text)] tracking-tight">Crosshair Zen Mode</h2>
                                        ...
                                        <input
                                            ...
                                            onChange={async (e) => {
                                                ...
                                                if (ytMatch && ytMatch[1]) {
                                                    ...
                                                    setAmbience({ type: 'custom_yt', url: videoId, playing: true, title: 'Loading MusicNote...' });
                            ...
                            <div className="relative z-[10] flex-1 max-w-5xl mx-auto w-full px-6 pt-12 pb-32 sm:pt-20 sm:pb-44 flex flex-col items-center">
                                {/* Floating NavigationArrow MapTrifold (ToC) */}
                                {toc.length > 0 && (
                                        ...
                                        <div className="relative flex items-center bg-slate-950 dark:bg-black backdrop-blur-3xl rounded-full border border-white/10 shadow-[0_20px_80px_rgba(0,0,0,0.9)] transition-all duration-500 ease-out w-14 h-14 hover:w-auto hover:h-[72px] hover:px-6 overflow-hidden group-hover:rounded-[2.5rem]">
                                            ...
                                            <div className="flex items-center justify-center shrink-0 w-14 h-14 relative z-10">
                                                ...
                                                <div className={`relative w-10 h-10 sm:w-11 sm:h-11 rounded-full bg-slate-800 flex items-center justify-center overflow-hidden border-2 transition-all duration-1000 ${ambience.playing ? 'animate-[spin_4s_linear_infinite]' : 'rotate-45'
                                                    ...
                                                    <MusicNote className={`text-white transition-all ${ambience.playing ? 'scale-110 opacity-70' : 'scale-90 opacity-20'
                                            ...
                                            <div className="flex items-center gap-6 opacity-0 group-hover:opacity-100 transition-all duration-500 delay-150 whitespace-nowrap">
                                                ...
                                                <div className="flex items-center gap-5 focus:outline-none">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <MusicNote className="w-3 h-3 text-white/20" />
                                                        <input
                                ...
                                <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[50] flex items-center gap-3 px-6 py-3 bg-[var(--color-surface)]/80 backdrop-blur-md border border-[var(--color-border)] rounded-full shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-1000">
                                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-[var(--color-text-muted)] opacity-60">
                                        <TextAlignLeft className="text-[var(--color-primary)]" />
                                        <span>{form.content?.replace(/<[^>]*>/g, ' ').replace(/&nbsp;/g, ' ').trim().split(/\s+/).filter(Boolean).length || 0} Kata</span>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-emerald-500">
                                        <CloudArrowUp className="animate-pulse" />
                                ...
                                <div className="text-center pt-16 pb-16 text-[9px] font-black text-[var(--color-text-muted)] opacity-20 uppercase tracking-[0.4em] pointer-events-none">
                                    Crosshair Mode • Kedamaian dalam Tulisan
                                </div>
                            </div>
                        </div>,
                        document.body
                ...
                <CommandPalette
                    ...
                    actions={[
                        { id: 'save', label: 'Simpan Informasi', icon: FloppyDisk, shortcut: 'Ctrl+S', action: () => handleSave() },
                        { id: 'media', label: 'Buka Media Library', icon: ImageSquare, shortcut: 'M', action: () => setIsMediaModalOpen(true) },
                        { id: 'zen', label: 'Toggle Zen Mode', icon: Moon, shortcut: 'Z', action: () => setIsZenMode(!isZenMode) },
                        { id: 'ai', label: 'Buka AI Assistant', icon: Sparkle, shortcut: 'Ctrl+J', action: () => setIsAiModalOpen(true) },
                        { id: 'history', label: 'Riwayat Revisi', icon: ArrowCounterClockwise, action: () => setShowRevisionHistory(true) },
                        { id: 'back', label: 'Kembali ke Daftar', icon: CaretLeft, action: handleGoBack },
                    ]}
                />
        ...
        <div className="fixed inset-0 z-[11000] flex items-start justify-center pt-24 px-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className="w-full max-w-xl bg-[var(--color-surface)] rounded-3xl border border-[var(--color-border)] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                <div className="p-4 border-b border-[var(--color-border)] flex items-center gap-3">
                    <Funnel className="text-[var(--color-text-muted)] opacity-30" />
...
function AiAssistant({ isOpen, onClose, content, title, onApply }) {
    ...
    const tones = [
        { label: 'Formal', val: 'FORMAL', icon: UserPen, color: 'text-blue-500', bg: 'bg-blue-500/5', hover: 'hover:bg-blue-500/10' },
        { label: 'Santai', val: 'SANTAI', icon: Coffee, color: 'text-amber-500', bg: 'bg-amber-500/5', hover: 'hover:bg-amber-500/10' },
        { label: 'Prof-Zen', val: 'PROFESSIONAL-ZEN', icon: Moon, color: 'text-purple-500', bg: 'bg-purple-500/5', hover: 'hover:bg-purple-500/10' },
    ]
        ...
        <Modal
            ...
            footer={
                <div className="flex items-center justify-center gap-2 w-full opacity-60">
                    <Info className="text-indigo-500 w-3 h-3" />
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {tones.map(t => (
                            <button key={t.val} onClick={() => handleGenerate(t.val)} className={`group relative h-9 rounded-xl border border-[var(--color-border)] ${t.bg} flex items-center justify-center gap-2 transition-all duration-300 ${t.hover} hover:border-indigo-500/30 active:scale-95`}>
                                <t.icon className={`text-[10px] ${t.color} opacity-40 group-hover:opacity-100 transition-opacity`} />
                ...
                <div className="space-y-3">
                    ...
                    >
                        ...
                        <div className="relative flex items-center justify-center gap-2">
                            {loading ? (
                                <Spinner className="animate-spin text-white w-3 h-3" />
                            ) : (
                                <>
                                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-white">Olah Konten</span>
                                    <ArrowRight className="text-white/40 w-3 h-3 group-hover:translate-x-1 transition-transform" />
                                </>
                            )}
                        </div>
                    </button>
        ...
        <div className="fixed inset-0 z-[11000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md animate-in fade-in duration-300" onClick={onClose}>
            <div className="w-full max-w-lg bg-[var(--color-surface)] rounded-[2rem] border border-[var(--color-border)] shadow-2xl p-8 animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
                <div className="flex items-center gap-4 mb-8">
                    <div className="w-12 h-12 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500 text-xl"><ArrowCounterClockwise /></div>
                    <div>
                        <h3 className="w-5 h-5 font-black text-[var(--color-text)]">Riwayat Revisi</h3>
