...
import { ArrowLeft, ArrowRight, CheckCircle, Info, MapPin, Moon, Sun, User, Users } from '@phosphor-icons/react'
...
export default function PublicEnrollmentPage() {
    ...
    useEffect(() => {
        document.title = 'Koperasi SenyumMu - Pendaftaran Online'
            ...
            <div className="w-full max-w-[420px] space-y-6 relative z-10">
                ...
                <div className="flex items-center justify-between">
                    <Link to="/" className="flex items-center gap-2.5 group">
                        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-accent)] flex items-center justify-center shadow-lg shadow-indigo-500/20 group-hover:scale-105 transition-transform">
                            <span className="text-white font-bold font-heading w-5 h-5">L</span>
                        </div>
                ...
                <div className="text-center sm:text-left mt-8 mb-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 flex-wrap">
                        <p className="w-4 h-4 text-[var(--color-text-muted)]">Gelombang Aktif: <span className="font-bold text-[var(--color-primary)]">{activeWave?.name}</span></p>
                        <Link to="/psb/status" className="w-3 h-3 font-bold text-[var(--color-primary)] hover:underline flex items-center gap-1">
                            Cek Status Kelulusan <ArrowRight className="w-3 h-3" />
                        </Link>
                    </div>
                </div>
                ...
                <div className="glass rounded-[2rem] p-5 sm:p-6 relative overflow-hidden shadow-2xl shadow-indigo-500/10 animate-in fade-in zoom-in duration-700">
                    ...
                    {step === 0 && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Nama Lengkap</label>
                            ...
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Jenis Kelamin</label>
                            ...
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Pilihan Program</label>
                                <div className="flex gap-3">
                                    <label className={`flex-1 flex flex-col items-center justify-center h-[52px] rounded-xl border cursor-pointer transition-all ${form.program === 'reguler' ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/10' : 'border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)]/50'}`}>
                                        ...
                                        <span className={`font-bold w-4 h-4 leading-none ${form.program === 'reguler' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}>Reguler</span>
                                    ...
                                    <label className={`flex-1 flex flex-col items-center justify-center h-[52px] rounded-xl border cursor-pointer transition-all ${form.program === 'boarding' ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/10' : 'border-[var(--color-border)] bg-[var(--color-surface)] hover:border-[var(--color-primary)]/50'}`}>
                                        ...
                                        <span className={`font-bold w-4 h-4 leading-none ${form.program === 'boarding' ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}>Boarding</span>
                            ...
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Asal Sekolah</label>
                    ...
                    {step === 1 && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Nama Ayah</label>
                            ...
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Nama Ibu</label>
                            ...
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">No. HP / WhatsApp (Aktif)</label>
                    ...
                    {step === 2 && (
                        <div className="space-y-3 animate-in fade-in slide-in-from-right-4 duration-500">
                            <div>
                                <label className="w-3 h-3 font-bold uppercase tracking-wider text-[var(--color-text-muted)] mb-1.5 block pl-1">Alamat Domisili</label>
                            ...
                            <div className="p-3.5 rounded-xl bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 flex gap-3 text-[var(--color-primary)] w-4 h-4 font-medium">
                                <Info className="mt-0.5 shrink-0" />
                                <p className="w-3 h-3 leading-relaxed text-[var(--color-text)]">Pastikan data sudah benar. Pihak pesantren akan menghubungi Anda via WhatsApp setelah data masuk.</p>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="flex items-center gap-3 mt-4 pt-4 border-t border-[var(--color-border)]/50">
                        {step > 0 && (
                            <button onClick={() => setStep(s => s - 1)} className="btn bg-[var(--color-surface-alt)] text-[var(--color-text)] border border-[var(--color-border)] hover:bg-[var(--color-border)] py-3 px-5 transition-all w-4 h-4 font-bold shadow-none">
                                <ArrowLeft />
                            </button>
                        )}
                        {step < 2 ? (
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 pt-4 animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-300">
                    ...
                    <Link to="/" className="w-3 h-3 font-bold text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-all flex items-center gap-2">
                        <span>â† Beranda</span>
                    </Link>
                </div>
            </div>
