            ...
            <div className="max-w-7xl mx-auto px-6 sm:px-6 lg:px-8">
                <div className="flex items-center justify-between relative">
                    ...
                    <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 items-center gap-8 bg-[var(--color-surface)]/50 dark:bg-[var(--color-surface-alt)]/50 backdrop-blur-md px-6 py-2.5 rounded-full border border-[var(--color-border)]">
                        <Link to="/" className="text-[var(--color-text-muted)] hover:text-[var(--color-primary)] font-medium w-4 h-4 transition-all hover:scale-105 active:scale-95">Beranda</Link>
                        <Link to="/informasi" className="text-[var(--color-primary)] font-black w-4 h-4">Informasi</Link>
                        <Link to="/check" className="text-[var(--color-text-muted)] hover:text-[var(--color-primary)] font-medium w-4 h-4 transition-all hover:scale-105 active:scale-95">Cek Poin</Link>
                    </div>
                    ...
                    <div className="hidden md:flex items-center gap-4 z-50">
                        ...
                        <Link to={user ? "/dashboard" : "/login"} className="inline-flex items-center px-5 py-2 w-3 h-3 font-black rounded-xl bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary-dark)] text-white shadow-lg shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all">
            ...
            {menuOpen && (
                <div className="md:hidden absolute top-full left-4 right-4 mt-2 bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl shadow-2xl p-3 space-y-1 animate-in fade-in duration-200">
                    {[['/', 'Beranda'], ['/informasi', 'Informasi'], ['/check', 'Cek Poin'], [user ? '/dashboard' : '/login', user ? 'Ke Dashboard' : 'Login Staff']].map(([to, label]) => (
                        <Link key={to} to={to} onClick={() => setMenuOpen(false)} className="flex items-center px-4 py-3 rounded-xl w-4 h-4 font-semibold text-[var(--color-text)] hover:bg-[var(--color-surface-alt)] transition-colors">{label}</Link>
                    ))}
                </div>
            )}
        </nav>
...
const DetailModal = memo(({ item, onClose }) => {
    ...
    const handlePrint = () => {
        ...
        <h1>${item.title}</h1>
        ${item.content}
        <div class="footer"><span>Diterbitkan oleh ${item.display_name || (item.author?.split('@')[0] || 'Admin')}</span><span>Koperasi SenyumMu</span></div>
        </body></html>`)
        w.document.close()
            ...
            <div className="relative w-full max-w-2xl bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {item.read_time && (
                                <span className="text-[10px] font-bold text-[var(--color-text-muted)] opacity-60 flex items-center gap-1">
                                    <Clock className="w-2 h-2" />{item.read_time} mnt baca
                                </span>
                            )}
                        </div>
                ...
                <div className="shrink-0 px-6 sm:px-10 py-4 border-t border-[var(--color-border)] bg-[var(--color-surface)]/80 backdrop-blur-sm flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="w-4 h-4 font-black text-[var(--color-text)]">{item.display_name || item.author?.split('@')[0] || 'Admin'}</p>
                    </div>
        ...
        <div onClick={() => onClick(item)} className="group bg-[var(--color-surface)] border border-[var(--color-border)] rounded-[2rem] overflow-hidden hover:shadow-xl hover:border-[var(--color-primary)]/30 transition-all duration-300 flex flex-col cursor-pointer">
            <div className="h-44 overflow-hidden shrink-0 bg-[var(--color-surface-alt)]">
                {item.image_url
                    ...
                    : <div className="w-full h-full flex items-center justify-center opacity-15"><Newspaper className="text-4xl" /></div>
                }
            </div>
            <div className="p-6 flex flex-col flex-1">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                    ...
                    {item.read_time && <span className="text-[9px] text-[var(--color-text-muted)] opacity-50 flex items-center gap-1"><Clock className="w-2 h-2" />{item.read_time} mnt</span>}
                </div>
                <h3 className="w-5 h-5 font-black text-[var(--color-text)] leading-tight mb-2 group-hover:text-[var(--color-primary)] transition-colors line-clamp-2">{item.title}</h3>
                ...
                <div className="flex items-center justify-between pt-4 mt-3 border-t border-[var(--color-border)]/50">
                    <span className="text-[9px] font-bold text-[var(--color-text-muted)] opacity-50 flex items-center gap-1">
                        <User className="w-2 h-2" />{item.display_name || item.author?.split('@')[0] || 'Admin'}
                    </span>
                    <span className="text-[10px] font-black text-[var(--color-primary)] flex items-center gap-1.5">Baca <ArrowRight className="w-3 h-3" /></span>
                </div>
            </div>
        </div>
    )
        ...
        <div className="min-h-screen bg-[var(--color-surface)] transition-colors duration-300">
            ...
            {newsBadge > 0 && (
                <div className="fixed top-20 left-1/2 -translate-x-1/2 z-40 w-full max-w-xl px-4 animate-in slide-in-from-top-2 duration-300">
                    <div className="flex items-center gap-3 px-4 py-3 bg-[var(--color-surface)] border border-[var(--color-primary)]/30 rounded-2xl shadow-xl shadow-black/10">
                        ...
                        <p className="w-4 h-4 font-bold text-[var(--color-primary)] flex-1">
            ...
            <div className="pt-28 pb-12 px-6 text-center relative overflow-hidden">
                ...
                <div className="relative max-w-2xl mx-auto">
                    ...
                    <p className="text-[var(--color-text-muted)] w-5 h-5 mb-8">
                        Update terkini seputar Koperasi SenyumMu, jadwal, dan pengumuman penting sekolah.
                    </p>
                    <div className="relative max-w-lg mx-auto">
                        <MagnifyingGlass className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)] w-4 h-4 pointer-events-none" />
            ...
            <div className="max-w-7xl mx-auto px-6 pb-24">
                ...
                {search && !loading && (
                    <div className="flex items-center gap-2 mb-6 w-4 h-4 text-[var(--color-text-muted)]">
                        Hasil untuk "<span className="font-bold text-[var(--color-text)]">{search}</span>" — {totalCount} ditemukan
                        <button onClick={() => { setInputVal(''); setSearch(''); setPage(1) }} className="ml-2 text-[var(--color-primary)] font-bold hover:underline w-3 h-3">Hapus</button>
                    </div>
                )}
            ...
            <footer className="border-t border-[var(--color-border)] py-6 px-6">
                <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-[var(--color-text-muted)]">
                    <Link to="/" className="font-heading font-bold w-5 h-5 text-[var(--color-text)]">Koperasi SenyumMu</Link>
                    <span>© 2026 Koperasi SenyumMu. All rights reserved.</span>
