...
export default function ClassesPage() {
    ...
    const handleExportPDF = async (filename, options = {}) => {
        ...
        try {
            ...
            doc.setFontSize(8)
            doc.text(`Tanggal: ${new Date().toLocaleDateString('id-ID')}  |  Total: ${allRows.length} kelas  |  Scope: ${exportScope === 'filtered' ? 'Funnel Aktif' : exportScope === 'selected' ? 'Dipilih' : 'Semua'}`, 14, 18)
            ...
            <div className="p-4 md:p-6 space-y-4 max-w-[1800px] mx-auto">
                ...
                {!canEdit && (
                    <div className="px-4 py-2.5 rounded-xl bg-rose-500/10 border border-rose-500/20 flex items-center gap-2">
                        ...
                        <p className="text-[11px] font-bold text-rose-600">Mode Read-only — Pen data kelas dinonaktifkan oleh administrator.</p>
                    </div>
                )}
                ...
                {selectedIds.length > 0 && (
                    <div className="mb-4 px-4 py-3 rounded-2xl bg-[var(--color-primary)]/10 border border-[var(--color-primary)]/20 flex items-center justify-between gap-3 flex-wrap animate-in slide-in-from-top-2">
                        <p className="w-4 h-4 font-black text-[var(--color-primary)] tracking-tight">{selectedIds.length} kelas dipilih</p>
                        <div className="flex items-center gap-2 flex-wrap">
                            <button onClick={() => setIsBulkDeleteOpen(true)} className="h-8 px-4 rounded-xl bg-red-500/10 text-red-600 text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all flex items-center gap-2"><Trash /> Hapus</button>
                            <button onClick={() => setSelectedIds([])} className="h-8 px-4 rounded-xl bg-[var(--color-surface-alt)] text-[var(--color-text-muted)] text-[10px] font-black uppercase tracking-widest hover:text-[var(--color-text)] transition-all flex items-center gap-2"><X /> Batal</button>
                        </div>
                    </div>
                )}
                ...
                <div className="glass rounded-[1.5rem] mb-4 border border-[var(--color-border)] overflow-hidden">
                    {/* Row 1: MagnifyingGlass + Quick Filters + Action Buttons */}
                    <div className="flex items-center gap-2 p-2.5 lg:p-3">
                        {/* MagnifyingGlass Bar - Dynamic & Responsive */}
                ...
                <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title="Hapus Kelas" size="sm">
                    <div className="space-y-6">
                        <div className="p-4 bg-red-500/10 rounded-2xl flex items-center gap-4 text-red-500 border border-red-500/20 shadow-inner">
                            <div className="w-12 h-12 rounded-xl bg-red-500/20 flex items-center justify-center shrink-0 text-xl border border-red-500/30 animate-pulse"><Trash /></div>
                        ...
                        <div className="flex gap-3 pt-2">
                            ...
                            <button onClick={handleDeleteConfirm} disabled={submitting} className="h-11 flex-[1.5] rounded-xl bg-red-500 hover:bg-red-600 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-red-500/20 transition-all">{submitting ? <Spinner className="animate-spin" /> : 'HAPUS PERMANEN'}</button>
                        </div>
                    </div>
                </Modal>
                ...
                <Modal isOpen={isBulkDeleteOpen} onClose={() => setIsBulkDeleteOpen(false)} title="Hapus Massal Kelas" size="sm">
                    <div className="space-y-6">
                        ...
                        <div className="flex gap-3 pt-2">
                            ...
                            <button onClick={handleBulkDelete} disabled={submitting} className="h-11 flex-[1.5] rounded-xl bg-red-500 hover:bg-red-600 text-white font-black text-[10px] uppercase tracking-widest shadow-lg shadow-red-500/20 transition-all">{submitting ? <Spinner className="animate-spin" /> : 'HAPUS SEMUA'}</button>
                        </div>
                    </div>
                </Modal>
