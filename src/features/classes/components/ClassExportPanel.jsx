...
const COLUMN_DEFS = [
    { key: 'nama_kelas', label: 'Nama Kelas', icon: Buildings, exportKey: 'Nama Kelas' },
    { key: 'tingkat', label: 'Tingkat / Grade', icon: GraduationCap, exportKey: 'Tingkat / Grade' },
    { key: 'program', label: 'Program / Major', icon: Buildings, exportKey: 'Program / Major' },
    { key: 'wali_kelas', label: 'Wali Kelas', icon: UserCheck, exportKey: 'Wali Kelas' },
    { key: 'tahun_ajaran', label: 'Tahun Ajaran', icon: Calendar, exportKey: 'Tahun Ajaran' },
    { key: 'jumlah_siswa', label: 'Jumlah Siswa', icon: Users, exportKey: 'Jumlah Siswa' },
]
...
const DEFAULT_COLUMNS = ['nama_kelas', 'tingkat', 'program']
...
export default function ClassExportPanel(props) {
    const {
        isOpen, onClose,
    ...
    const scopeOptions = useMemo(() => [
        { val: 'filtered', label: 'Filter Aktif', desc: `${classes.length} kelas`, icon: SlidersHorizontal, disabled: false },
        ...
        <div className="space-y-4">
            ...
            <div className="p-3 rounded-xl bg-[var(--color-surface-alt)]/50 border border-[var(--color-border)]">
                <div className="text-[10px] font-bold text-[var(--color-text-muted)]">
                    Ringkasan: <span className="font-black text-[var(--color-text)]">{exportPreviewData.total}</span> kelas akan diekspor sebagai <span className="font-black text-[var(--color-text)]">{exportColumns.length}</span> kolom
                </div>
            </div>
        </div>
    )
        ...
        <div className="flex flex-col min-h-[calc(100vh-3.5rem)]">
            <div className="px-5 pt-5 pb-3 shrink-0">
                ...
                <div>
                    ...
                    <p className="text-[var(--color-text-muted)] text-[10px] mt-0.5 font-medium">Cadangkan atau pindahkan data kelas ke format CSV, Excel, atau PDF.</p>
                </div>
            </div>
