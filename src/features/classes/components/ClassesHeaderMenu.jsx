...
const ClassesHeaderMenu = memo(function ClassesHeaderMenu({
    isOpen, rect, mounted, canEdit, isMutating, onClose,
    onImportClick, onExportClick, onArchivedClick,
    archivedCount = 0, fetchArchived,
}) {
    ...
    const items = [
        { id: 'import', label: 'Import CSV / Excel', desc: 'Unggah data kelas masal dari file Excel/CSV', icon: FileArrowDown, color: 'emerald', shortcut: `${MOD}+I`, disabled: !canEdit, onClick: onImportClick },
        { id: 'export', label: 'Export Data', desc: 'Cadangkan seluruh database ke format Excel', icon: FileArrowUp, color: 'amber', shortcut: `${MOD}+E`, disabled: false, onClick: onExportClick },
        null,
        { id: 'archived', label: 'Arsip Kelas', desc: 'Lihat & pulihkan data kelas tidak aktif', icon: Archive, color: 'orange', badge: archivedCount, disabled: false, onClick: () => { fetchArchived?.(); onArchivedClick?.(); } },
    ]
    ...
    return createPortal(
        ...
        getPortalContainer('portal-classes-header-menu')
    )
})

export default ClassesHeaderMenu
