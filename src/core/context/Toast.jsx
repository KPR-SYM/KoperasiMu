...
const TOAST_TYPES = {
    success: { icon: CheckCircle, bg: 'bg-emerald-600', border: 'border-emerald-400/30' },
    error: { icon: XCircle, bg: 'bg-red-600', border: 'border-red-400/30' },
    info: { icon: Info, bg: 'bg-indigo-600', border: 'border-indigo-400/30' },
    warning: { icon: Warning, bg: 'bg-amber-500', border: 'border-amber-300/30' },
    undo: { icon: CheckCircle, bg: 'bg-[#3730a3]', border: 'border-indigo-400/30' },
    pdf: { icon: FileText, bg: 'bg-rose-600', border: 'border-rose-400/30' },
    upload: { icon: CloudArrowUp, bg: 'bg-sky-600', border: 'border-sky-400/30' },
}
...
export function ToastProvider({ children }) {
    ...
    // (clearTimeout) Saat Toast Ditutup Manual Atau Saat ArrowCounterClockwise Dipilih
    ...
    // (Misalnya Saat Aksi ArrowCounterClockwise Sudah Pasti Tidak Mungkin Dipakai Lagi)
    ...
    return (
        <ToastContext.Provider value={contextValue}>
            ...
            <div className="fixed top-4 right-4 left-4 sm:left-auto z-[99999] flex flex-col gap-2 sm:max-w-sm">
                {toasts.map(toast => {
                    ...
                    return (
                        ...
                        >
                            {/* Progress Bar Yang Menyusut Mengikuti Sisa Waktu Sebelum Toast ArrowCounterClockwise Otomatis Tertutup */}
                            {isUndo && (
                                <div
            ...
            {/* Keyframe Animasi "shrink" Untuk Progress Bar Toast ArrowCounterClockwise Di Atas — Lebar Menyusut Dari 100% Ke 0% */}
            <style>{`
                @keyframes shrink {
                    from { width: 100%; }
                    to { width: 0%; }
