...
export function StatCard({ 
    ...
    return (
        <div 
            onClick={onClick} 
            title={title} 
            className={`shrink-0 snap-center w-[180px] xs:w-[200px] sm:w-auto glass group relative overflow-hidden rounded-xl border-t ${finalBorderTop} border border-[var(--color-border)] p-3 flex items-center gap-3 hover:border-[var(--color-primary)]/30 hover:shadow-lg hover:shadow-[var(--color-primary)]/5 transition-all duration-300 min-h-[68px] ${onClick ? 'cursor-pointer active:scale-95' : ''} ${activeRing} ${className}`}
        >
            ...
            {/* Icon Container */}
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${finalBg} transform group-hover:scale-110 transition-all duration-500`}>
            ...
            <div className="min-w-0 relative z-10 flex-1 text-start">
                ...
                {loading
                    ...
                    : <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--color-text-muted)] opacity-70 mb-0.5 truncate">{label}</p>
                }
