...
const normalizeArabicDigits = (str) => {
    ...
    return str.replace(/[Ù -Ù©]/g, d => String(arabicDigits.indexOf(d)))
}
...
const RichDatePicker = memo(({
    ...
    const renderCalendar = () => (
        <div className={`p-2 bg-[var(--color-surface)] ${isMobile ? 'w-full max-w-sm px-4' : 'w-[210px]'}`}>
            {/* Header: Month & Year Selector */}
            <div className="flex items-center justify-between mb-2 pb-1 border-b border-[var(--color-border)]/40 select-none">
                <button
                    type="button"
                    onClick={handlePrevMonth}
                    className="w-6 h-6 rounded-md hover:bg-[var(--color-surface-alt)] text-[var(--color-text)] flex items-center justify-center transition-colors active:scale-90"
                >
                    <CaretLeft className="w-2.5 h-2.5 opacity-70" />
                </button>
                <span className="text-[9px] font-black uppercase tracking-[0.15em] text-[var(--color-text)]">
                ...
                <button
                    ...
                    className="w-6 h-6 rounded-md hover:bg-[var(--color-surface-alt)] text-[var(--color-text)] flex items-center justify-center transition-colors active:scale-90"
                >
                    <CaretRight className="w-2.5 h-2.5 opacity-70" />
                </button>
            </div>
                ...
                {calendarGrid.map((day, idx) => {
                    ...
                    return (
                        <button
                            ...
                            className={`w-full aspect-square max-w-[34px] max-h-[34px] mx-auto rounded-md text-[10px] font-bold transition-all relative flex items-center justify-center ${selected
            ...
            <div className="flex items-center justify-between pt-2 border-t border-[var(--color-border)] select-none">
                ...
                <button
                    type="button"
                    onClick={handleToday}
                    className="text-[8px] font-black uppercase tracking-widest text-[var(--color-primary)] hover:bg-[var(--color-primary)]/5 px-1.5 py-1 rounded-lg transition-all border border-transparent"
                >
                    {currentLocale.today}
                </button>
            </div>
    ...
    return (
        <div className={`relative ${className}`} ref={ref}>
            <input
                ...
                className={`w-full pl-8 pr-7 h-9 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] focus:border-[var(--color-primary)] focus:bg-[var(--color-surface)] outline-none transition-all text-[10px] font-black shadow-sm ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
            />

            <Calendar
                className="absolute left-3 top-1/2 -translate-y-1/2 w-3 h-3 text-[var(--color-text-muted)] opacity-50 pointer-events-none"
            />

            <CaretDown
                className={`absolute right-3 top-1/2 -translate-y-1/2 w-3 h-3 opacity-40 transition-transform duration-300 pointer-events-none ${isOpen ? 'rotate-180 text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`}
            />
