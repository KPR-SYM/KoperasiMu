...
const RichSelect = memo(({
    ...
    // UsersThree options dynamically
                ...
                <div className="p-2 border-b border-[var(--color-border)] bg-[var(--color-surface-alt)]/30 shrink-0">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Cari..."
                            className="w-full h-7 pl-7 pr-7 rounded-lg border-[var(--color-border)] bg-[var(--color-surface)] text-[10px] focus:border-[var(--color-primary)] focus:ring-0 outline-none transition-all font-bold"
                        />
                        {search && (
                            <button
                                onClick={() => setSearch('')}
            ...
            {uniqueGroups.length > 0 && (
                <div className="flex items-center gap-1 px-2 py-1 border-b border-[var(--color-border)]/40 bg-[var(--color-surface-alt)]/20 overflow-x-auto shrink-0 select-none custom-scrollbar">
                    <button
                        ...
                        className={`px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider transition-all border ${activeGroup === 'All'
                    ...
                    {uniqueGroups.map(g => {
                        ...
                        return (
                            <button
                                ...
                                className={`px-2.5 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider transition-all border shrink-0 ${isActive
                                        ? activeClasses
                                        : 'bg-[var(--color-surface)] text-[var(--color-text-muted)] border-[var(--color-border)] hover:bg-[var(--color-surface-alt)]'
                                    }`}
                            >
            ...
            >
                {extraOption && (
                    <button
                        ...
                        className={`w-full text-left px-3 py-1 text-[10px] font-bold hover:bg-[var(--color-primary)]/5 transition-all flex items-center justify-between group whitespace-nowrap ${value === extraOption.id ? 'text-[var(--color-primary)] bg-[var(--color-primary)]/5' : 'text-amber-600'}`}
                    >
                ...
                {allowCustom && search && !options.some(o => o.name?.toLowerCase() === search.toLowerCase()) && (
                    <button
                        ...
                        className="w-full text-left px-3 py-1 text-[10px] font-bold text-[var(--color-primary)] bg-[var(--color-primary)]/5 hover:bg-[var(--color-primary)]/10 transition-all flex items-center justify-between group whitespace-nowrap border-b border-[var(--color-border)]"
                    >
                ...
                {filteredOptions.length === 0 ? (
                    ...
                    Object.keys(groupedOptions).map(groupName => (
                        <div key={groupName} className="flex flex-col">
                            ...
                            <div className={`px-3 py-1 text-[8px] font-black uppercase tracking-widest border-y border-[var(--color-border)]/35 sticky top-0 backdrop-blur-md z-10 select-none ${groupName.toLowerCase().includes('pelanggaran')
    ...
    const hasCustomHeight = buttonClassName && /\bh-\[?\w+\]?\b/.test(buttonClassName)
    let heightClass = ''
    let paddingClass = ''
