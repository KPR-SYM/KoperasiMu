  ...
  return createPortal(
    ...
    >
      <div className="relative">
        <div className="relative bg-[#0f172a]/90 backdrop-blur-3xl border border-white/20 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] px-1.5 py-1.5 flex items-center justify-between gap-2 text-white overflow-hidden">
          {/* Animated scanline */}
          <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent -translate-x-full animate-[shimmer_2s_infinite]" />

          {/* Count Indicator */}
          <div className="flex items-center gap-2 pl-1.5 rtl:pl-0 rtl:pr-1.5 shrink-0">
            <div className="w-8 h-8 rounded-lg bg-[var(--color-primary)] flex items-center justify-center font-black text-xs shadow-lg shadow-[var(--color-primary)]/30 shrink-0">
              {selectedCount}
            </div>
          ...
          {/* Left Divider */}
          <div className="w-px h-5 bg-white/10 mx-1 hidden sm:block shrink-0" />

          {/* Actions Area */}
          <div className="flex items-center gap-1.5 flex-1 justify-center max-sm:[&_span]:hidden max-sm:[&_button]:w-8 max-sm:[&_button]:h-8 max-sm:[&_button]:px-0 max-sm:[&_button]:rounded-lg">
            {children}
          </div>
          ...
          <div className="flex items-center pr-1 rtl:pr-0 rtl:pl-1 shrink-0">
            ...
            <button
              onClick={onClear}
              className="w-8 h-8 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 hover:text-white transition-all flex items-center justify-center translate-x-0 active:scale-90"
