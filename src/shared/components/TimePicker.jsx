...
const TimePicker = memo(({
    ...
    const renderClockDropdown = () => (
        <>
            ...
            <div className="px-4 py-2 flex-1 flex flex-col justify-center">
                <div className="flex items-center justify-center gap-6 sm:gap-3 py-1.5 bg-[var(--color-surface-alt)]/60 border border-[var(--color-border)]/45 rounded-xl relative shadow-inner h-[76px]">
                    {/* Hour adjusters */}
                    <div className="flex flex-col items-center">
                        <button
                            type="button"
                            onClick={() => adjustHour(1)}
                            className="w-7 h-4 flex items-center justify-center text-[9px] text-[var(--color-text-muted)] hover:text-[var(--color-primary)] hover:bg-[var(--color-primary)]/5 rounded transition-colors active:scale-75"
                        >
                            <CaretUp className="w-2.5 h-2.5" />
                        </button>
                        <span className="text-xl font-black tracking-wider text-[var(--color-text)] select-none">
                    ...
                    <span className="text-lg font-black text-[var(--color-text-muted)] pb-1 select-none">:</span>
                    ...
                    {!is24h && (
                        <button
                            ...
                            className="ml-1 px-2 py-1 text-[10px] font-black tracking-widest text-white bg-[var(--color-primary)] rounded-lg shadow-sm hover:brightness-110 active:scale-90 transition-all select-none"
                        >
                            {timeDisplay.period}
                        </button>
                    )}
            ...
            <div className="p-1.5 border-t border-[var(--color-border)]/45 bg-[var(--color-surface-alt)]/15 h-[38px] shrink-0 flex items-center gap-1.5">
                {clearable && (
                    <button
                        type="button"
                        onClick={handleClear}
                        className="px-2 h-7 rounded-lg border border-red-200 bg-red-50 text-red-500 hover:bg-red-100 text-[9px] font-black tracking-widest uppercase transition-all shrink-0"
                    >
                        {currentLocale.clear}
                    </button>
                )}
                <button
                    ...
                    className="px-2 h-7 rounded-lg border border-[var(--color-border)] hover:bg-[var(--color-surface-alt)] text-[var(--color-text-muted)] hover:text-[var(--color-primary)] text-[9px] font-black tracking-widest uppercase transition-colors shrink-0"
                >
                    {is24h ? '24H' : '12H'}
                </button>
                <button
                    type="button"
                    onClick={handleDone}
                    className="flex-1 h-7 rounded-lg bg-[var(--color-primary)] text-white text-[9px] font-black uppercase tracking-widest hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-1 shadow-sm"
                >
    ...
    return (
        <div className={`relative ${className}`} ref={ref}>
            ...
            <CaretDown 
                className={`absolute right-3.5 top-1/2 -translate-y-1/2 w-2.5 h-2.5 opacity-40 transition-transform duration-300 pointer-events-none ${isOpen ? 'rotate-180 text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]'}`} 
            />

            {/* Premium Headerless Highly-Readable Dropdown */}
            {isOpen && createPortal(
                isMobile ? (
                    <>
                        <div
                        ...
                        />
                        <div
                            className="fixed bottom-0 left-0 right-0 z-[99999] bg-[var(--color-surface)] border-t border-[var(--color-border)] rounded-t-2xl shadow-2xl animate-in slide-in-from-bottom duration-200 flex flex-col select-none"
                            onMouseDown={(e) => {
                                e.stopPropagation()
                                e.preventDefault()
                            }}
                        ...
                        >
                            ...
                            <div className="flex justify-center w-full pb-4">
                                <div className="w-full max-w-[280px] h-[150px] flex flex-col justify-between">
                                    {renderClockDropdown()}
                                </div>
                            </div>
                        </div>
                    ...
                    <div
                        className={`fixed z-[99999] w-[180px] h-[140px] bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl shadow-2xl overflow-hidden animate-in fade-in ${coords.placement === 'top' ? 'slide-in-from-bottom-3' : 'slide-in-from-top-3'} select-none flex flex-col justify-between`}
                        onMouseDown={(e) => {
                            e.stopPropagation()
                            e.preventDefault()
                        }}
                        style={{
                            left: Math.max(16, Math.min(coords.right - 180, window.innerWidth - 180 - 16)),
