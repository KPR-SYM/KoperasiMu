            ...
            */}
            ...
            >
                ...
                >
                    ...
                    {isBottomSheet && (
                        <div className="shrink-0 w-12 h-1.5 bg-[var(--color-text-muted)] rounded-full mx-auto mt-3 mb-1 md:hidden opacity-20" />
                    )}

                    {/* Header */}
                    <div className={`shrink-0 flex items-center justify-between px-5 md:px-6 ${isBottomSheet ? 'pt-3 pb-3' : 'py-4'} border-b border-[var(--color-border)]/50 bg-[var(--color-surface)] sticky top-0 z-10`}>
                        <div className="flex items-center gap-3 min-w-0">
                            {icon && (
                                <div className={`w-8 h-8 rounded-lg ${iconBg || 'bg-[var(--color-primary)]/10'} flex items-center justify-center shrink-0 ${iconColor || 'text-[var(--color-primary)]'}`}>
                                    {(() => {
                                        const IconComp = icon
                            ...
                            <div className="min-w-0">
                                <h3 className={`font-black font-heading tracking-tight text-[var(--color-text)] leading-tight ${icon ? 'text-[14px]' : 'text-base md:text-lg'}`}>
                                    {title}
                                </h3>
                                {description && (
                                    <div className="text-[9px] font-bold text-[var(--color-text-muted)] leading-relaxed mt-0.5 opacity-50">
                                        {description}
                                    </div>
                                )}
                            </div>
                        ...
                        <button
                            onClick={onClose}
                            className="w-8 h-8 text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-alt)] rounded-lg transition-colors active:scale-90 flex items-center justify-center shrink-0"
                    ...
                    {/* Content Area */}
                    <div
                        className={`flex-1 min-h-0 overflow-y-auto custom-scrollbar ${noPadding ? '' : 'px-5 md:px-6 py-4'} ${contentClassName}`}
                        style={{
                    ...
                    {/* Footer Area */}
                    {footer && (
                        <div className="shrink-0 px-5 md:px-6 py-4 border-t border-[var(--color-border)]/50 bg-[var(--color-surface-alt)]/30">
                            {footer}
                        </div>
                    )}
                </div>
